...
def _setup_workspace():
    ...
    Mirrors a real LightAgent workspace closely enough that the model has genuine
...
def scenario_memory_preference():
    ...
    def edit(ws):
        ...
        p.write_text((p.read_text(encoding="utf-8") if p.exists() else "") + "- Prefers concise replies; avoid long explanations\n", encoding="utf-8")
...
def scenario_memory_correction():
    ...
    def edit(ws):
        from datetime import datetime
        p = ws / "memory" / (datetime.now().strftime("%Y-%m-%d") + ".md")
        p.write_text((p.read_text(encoding="utf-8") if p.exists() else "") + "- Correction: team size is now 15 (was recorded as 8)\n", encoding="utf-8")
...
def scenario_skill_gap():
    ...
    def edit(ws):
        ...
        p.write_text(p.read_text(encoding="utf-8") + "4. List risk items / blockers.\n", encoding="utf-8")
...
def scenario_skill_error():
    ...
    def edit(ws):
        ...
        p.write_text(p.read_text(encoding="utf-8").replace("`$` prefix", "`¥` prefix (RMB)"), encoding="utf-8")
...
def scenario_skill_wrong_config():
    ...
    def edit(ws):
        ...
        p.write_text(
            p.read_text(encoding="utf-8").replace("api.example-wrong.com", "api.example-correct.com"),
            encoding="utf-8",
        )
        ...
        try:
            ...
            if sc["expect_evolved"]:
                ...
                if "expect_memory_contains" in sc:
                    ...
                    mem = daily.read_text(encoding="utf-8") if daily.exists() else ""
                ...
                if "expect_skill_contains" in sc:
                    ...
                    content = (ws / "skills" / sk / "SKILL.md").read_text(encoding="utf-8")
                ...
                prot = (ws / "skills" / "image-generation" / "SKILL.md").read_text(encoding="utf-8")
...
def _verify_undo():
    ...
    try:
        ...
        restored = mem.read_text(encoding="utf-8")
...
if __name__ == "__main__":
    if "--debug" in sys.argv:
        import logging
        from common.log import logger as _lightagent_logger
        _lightagent_logger.setLevel(logging.DEBUG)
        for _h in _lightagent_logger.handlers:
