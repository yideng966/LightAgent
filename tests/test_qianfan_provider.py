...
class TestQianfanConstantsAndRouting(unittest.TestCase):
    ...
    def test_lightagent_cli_routes_ernie_models_to_qianfan(self):
        from common import const
        import plugins

        old_plugin_path = plugins.instance.current_plugin_path
        lightagent_cli_was_registered = "LIGHTAGENT_CLI" in plugins.instance.plugins
        old_lightagent_cli_plugin = plugins.instance.plugins.get("LIGHTAGENT_CLI")
        parent_had_lightagent_cli = hasattr(plugins, "lightagent_cli")
        old_parent_lightagent_cli = getattr(plugins, "lightagent_cli", None)
        ...
        try:
            ...
            lightagent_cli_plugin = plugins.instance.plugins["LIGHTAGENT_CLI"]
        finally:
            plugins.instance.current_plugin_path = old_plugin_path
            if lightagent_cli_was_registered:
                plugins.instance.plugins["LIGHTAGENT_CLI"] = old_lightagent_cli_plugin
            else:
            ...
            if parent_had_lightagent_cli:
                plugins.lightagent_cli = old_parent_lightagent_cli
            elif hasattr(plugins, "lightagent_cli"):
                delattr(plugins, "lightagent_cli")

        self.assertEqual(
            lightagent_cli_plugin._resolve_bot_type_for_model("ernie-4.5-turbo-128k"),
            const.QIANFAN,
        )
        self.assertEqual(
            lightagent_cli_plugin._resolve_bot_type_for_model("qianfan"),
            const.QIANFAN,
        )
