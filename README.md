...
- Web 控制台提供定时任务管理 UI。
- 个人微信群任务可在编辑页面查看当前目标群，并切换到通道设置中已选择的其他稳定群。
...
- 自由回复：非 @ 消息进入队列，按活跃档位、本地规则、LLM 判断、情绪和时间规则决定是否回复；自由回复群可按稳定群 ID 单独选择档位，未指定时继承全局档位，四套档位参数继续全局共享。
...
新建工作区的默认 `AGENT.md` 和微信群内置人设不再要求使用 emoji 或 `[破涕为笑]`、`[吃瓜]` 等微信文字表情。该调整只影响文字回复风格，不改变表情包的自动收集、主动回复频率、检索或发送能力；已有 `config.json`、工作区 `AGENT.md` 和自定义人设不会在升级时被覆盖。
