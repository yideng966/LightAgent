...
- 具体知识内容可通过工具按需读取或检索。
- Web 控制台“知识”页支持在全局 Markdown 知识与群知识之间切换；全局知识保留分类、导入和图谱能力，群知识按已确认的稳定群 ID 独立查询和管理。
- 微信群 Agent 的群知识上下文与 `wechat_group_memory_search` 工具由服务端绑定当前群，工具参数不能切换到其他群。
...
- 自由回复图片理解独立开关，默认关闭，避免自动增加视觉模型调用；开启后，普通表情包会先转换为安全短语义，再由既有自由回复规则决定是否接话，不再因媒体类型自动回复。
...
在“群聊 -> 进退群消息”中分别配置入群欢迎和离群通知，两类全局开关默认开启，并在保存后写入 `config.json`；旧配置缺少对应开关字段时也按开启处理，显式关闭仍会保留。消息只对 `wechat_group_stable_room_ids` 中已选择且稳定身份已确认的群生效；按群自定义可以在全局关闭时独立启用。文本模式支持以下白名单占位符：

- 通用：`{room_name}`、`{member_name}`、`{member_names}`、`{member_count}`、`{event_time}`、`{bot_name}`。
- 入群专用：`{inviter_name}`。
- 离群专用：`{remover_name}`。
...
新表情包自动收藏本身不会调用 Vision；无法从文件名得到语义时先使用“群聊表情包”占位。仅当管理员开启“自由回复图片理解”且普通表情包需要进入自由回复判定时，系统会同步生成一条安全短语义并回写素材描述与群聊归档；未命中既有自由回复规则时不会发言。管理员仍可在 Web UI 人工修改描述或主动启动批量图片理解；批量任务只处理当前选中的稳定群和明确的待理解项，不覆盖已有正常语义或处理期间新保存的人工描述。
...
- 真实扫码、入群、离群、真实 mention 和跨群隔离仍需要人工链路验证；进退群事件不进入 `Context`、Bridge、Agent、群归档、记忆或画像链路。
