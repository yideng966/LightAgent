...
HELP_TEXT = """Usage: lightagent COMMAND [ARGS]...

  LightAgent CLI - Manage your LightAgent instance.

Commands:
  help     Show this message.
  version  Show the version.
  start    Start LightAgent.
  stop     Stop LightAgent.
  restart  Restart LightAgent.
  update   Update LightAgent and restart.
  status   Show LightAgent running status.
  logs     View LightAgent logs.
  skill    Manage LightAgent skills.
...
class LightAgentCLI(click.Group):
...
@click.group(cls=LightAgentCLI, invoke_without_command=True, context_settings=dict(help_option_names=[]))
@click.pass_context
def main(ctx):
    """LightAgent CLI - Manage your LightAgent instance."""
...
def version():
    """Show the version."""
    click.echo(f"lightagent {__version__}")
