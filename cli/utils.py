"""Shared utilities for lightagent CLI."""
...
def get_project_root() -> str:
    """Get the LightAgent project root directory."""
    # cli/ is directly under the project root
    return os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def get_workspace_dir() -> str:
    """Get the agent workspace directory from config, defaulting to ~/lightagent."""
...
def get_cli_language() -> str:
    ...
    Reads the `lightagent_lang` field from config.json (defaults to "auto") and runs
    ...
    try:
        ...
        configured = load_config_json().get("lightagent_lang", "auto")
