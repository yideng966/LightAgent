"""lightagent skill - Skill management commands."""
...
def _clone_repo(git_url: str):
    ...
    """
    tmp_dir = tempfile.mkdtemp(prefix="lightagent-skill-")
...
def _print_install_success(name: str, source: str):
    ...
    # so it works when `lightagent` is invoked from outside the project directory.
...
def _list_remote(page: int = 1):
    ...
    nav_parts = []
    if page > 1:
        nav_parts.append(f"lightagent skill list --remote --page {page - 1}")
    if page < total_pages:
        nav_parts.append(f"lightagent skill list --remote --page {page + 1}")
    if nav_parts:
...
def search(query):
    ...
    click.echo(f"\n  Install with: lightagent skill install <name>\n")
...
# ------------------------------------------------------------------
# lightagent skill install (CLI thin wrapper)
# ------------------------------------------------------------------
@skill.command()
@click.argument("name")
def install(name):
    ...
    Examples:
      ...
      lightagent skill install pptx                          (from Skill Hub)

      lightagent skill install larksuite/cli                 (GitHub shorthand, all skills)

      lightagent skill install larksuite/cli#skills/lark-im  (single skill by subpath)

      lightagent skill install https://github.com/owner/repo

      lightagent skill install https://gitlab.com/org/repo

      lightagent skill install git@github.com:owner/repo.git

      lightagent skill install ./my-local-skills             (local directory)
    ...
    if "application/json" in content_type:
        ...
        if source_type == "github":
            ...
            if len(result.installed) == installed_before:
                _register_installed_skill(name, source="skill_hub", display_name=hub_display_name)
                result.installed.append(name)
...
# ------------------------------------------------------------------
# lightagent skill enable / disable
# ------------------------------------------------------------------
@skill.command()
@click.argument("name")
def enable(name):
