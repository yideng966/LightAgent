"""lightagent install-browser - Install Playwright + Chromium for the browser tool."""
...
def run_install_browser(
    ...
    Reused by ``lightagent install-browser`` CLI and chat ``/install-browser``.
    ...
    stream("")
    stream("Browser tool ready! Restart LightAgent to enable it.", "green")
    _phase(on_phase, _t(
        "🎉 全部步骤结束。请重启 LightAgent 后使用 browser 工具。",
        "🎉 All steps finished. Restart LightAgent to use the browser tool.",
    ))
    return 0
