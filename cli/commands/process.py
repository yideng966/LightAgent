"""lightagent start/stop/restart/status/logs - Process management commands."""
...
def _get_pid_file():
    return os.path.join(get_project_root(), ".lightagent.pid")
...
def start(foreground, no_logs):
    ...
    pid = _read_pid()
    if pid:
        click.echo(f"LightAgent is already running (PID: {pid}).")
        return
    ...
    if foreground:
        click.echo("Starting LightAgent in foreground...")
        if _IS_WIN:
        ...
        click.echo("Starting LightAgent...")
        ...
        _write_pid(proc.pid)
        click.echo(click.style(f"✓ LightAgent started (PID: {proc.pid})", fg="green"))
...
def stop():
    ...
    pid = _read_pid()
    if not pid:
        click.echo("LightAgent is not running.")
        return

    click.echo(f"Stopping LightAgent (PID: {pid})...")
    try:
        _kill_pid(pid)
        for _ in range(30):
            time.sleep(0.1)
    ...
    _remove_pid()
    click.echo(click.style("✓ LightAgent stopped.", fg="green"))
...
def self_restart():
    ...
    own code), not by users — so it is hidden from `lightagent help`. Unlike `restart`,
    ...
    """
    if _IS_WIN:
        click.echo("self-restart is not supported on Windows; use `lightagent restart`.", err=True)
        sys.exit(1)
...
def update(ctx):
    ...
    if _IS_WIN:
        # On Windows, `lightagent.exe` (this process) locks the exe file, so
        # `pip install -e .` fails with WinError 5.  Write a small .bat
        # helper that waits for lightagent.exe to exit, then installs & starts.
        bat = os.path.join(root, "_lightagent_update.bat")
        lines = [
            "@echo off",
            "chcp 65001 >nul",
            "echo Waiting for lightagent.exe to exit...",
        ...
        lines += [
            "echo Reinstalling lightagent CLI...",
            f'"{python}" -m pip install -e . -q',
            "echo Starting LightAgent...",
        ...
        subprocess.Popen(
            ["cmd.exe", "/c", "start", "LightAgent Update", "/wait", bat],
            cwd=root,
        )
        click.echo(click.style(
        ...
        click.echo("Reinstalling lightagent CLI...")
        subprocess.call(
...
@click.command()
def status():
    """Show LightAgent running status."""
    ...
    # ModuleNotFoundError when `lightagent` runs from outside the project dir.
    get_cli_language()  # resolve lightagent_lang so i18n.t reflects config
    from common import i18n
    _t = i18n.t

    pid = _read_pid()
    if pid:
        click.echo(click.style(f"● LightAgent is running (PID: {pid})", fg="green"))
    else:
        click.echo(click.style("● LightAgent is not running", fg="red"))

    click.echo(_t(f"  版本: v{__version__}", f"  Version: v{__version__}"))

    # Project path bound to this `lightagent` CLI — disambiguates which checkout the
