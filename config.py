...
available_setting = {
    ...
    "model_fallbacks": [],  # optional chat fallback candidates, e.g. [{"bot_type": "custom:backup", "model": "backup-model"}]
    "model_failover_failure_threshold": 3,  # consecutive transient primary failures before opening the circuit
    "model_failover_cooldown_seconds": 300,  # seconds to use fallbacks before probing the primary again
    ...
    "image_create_prefix": ["画"],  # prefixes that enable image replies
    ...
    "wechat_group_sticker_reply_percent": 20,
    ...
    "wechat_group_sticker_online_provider": "xiaoapi",
    "wechat_group_sticker_online_endpoint": "https://api.suol.cc/v1/meme.php",
    "wechat_group_sticker_online_allowed_domains": ["biaoqing.gtimg.com", "tugelepic.mse.sogou.com"],
    "wechat_group_sticker_online_allow_gif": True,
    "wechat_group_sticker_online_search_count": 10,
    "wechat_group_sticker_cooldown_seconds": 30,
    "wechat_group_voice_reply_enabled": False,
    "wechat_group_voice_interaction_mode": "force_reply",
    ...
    "weixin_credentials_path": "~/.weixin_lightagent_credentials.json",  # credentials file path
    ...
    "agent_workspace": "~/lightagent",  # agent workspace path, used to store skills, memory, etc.
    ...
    "reasoning_effort": "low",  # Unified reasoning depth: low / medium / high / max
    "custom_thinking_protocol": "none",  # Thinking protocol for the legacy custom provider
...
def load_config():
    ...
    # the desktop build points LIGHTAGENT_DATA_DIR at ~/.lightagent so config survives updates.
    ...
    # Preserve the user's language during the product rename. The legacy key
    # is read once into the canonical key and is never used as a new default.
    if "lightagent_lang" not in config and "cow_lang" in config:
        config["lightagent_lang"] = config.pop("cow_lang")
    ...
    resolved_lang = i18n.resolve_language(config.get("lightagent_lang", "auto"))
    ...
    if config.get("agent", True):
        workspace = config.get("agent_workspace", "~/lightagent")
...
def get_data_root():
    ...
    The desktop build sets LIGHTAGENT_DATA_DIR (e.g. ~/.lightagent) so data lives in the
    ...
    """
    data_dir = os.environ.get("LIGHTAGENT_DATA_DIR")
    if data_dir:
...
def get_weixin_credentials_path():
    ...
    packaged desktop build (LIGHTAGENT_DATA_DIR set) keeps it under the data dir
    (~/.lightagent) so all user data stays together, while source deployments retain
    the legacy ~/.weixin_lightagent_credentials.json default unchanged.
    """
    configured = conf().get("weixin_credentials_path")
    if configured:
        return os.path.expanduser(configured)
    if os.environ.get("LIGHTAGENT_DATA_DIR"):
        return os.path.join(get_data_root(), "weixin_credentials.json")
    return os.path.expanduser("~/.weixin_lightagent_credentials.json")
