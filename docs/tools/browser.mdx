...
Works out of the box. Login state is saved under `~/.lightagent/browser_profile`. No configuration needed.
...
<Tabs>
  <Tab title="macOS">
    ...
    "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
      ...
      --user-data-dir="$HOME/.lightagent/chrome-cdp"
    ```
  </Tab>
  <Tab title="Linux">
    ```bash
  ...
  <Tab title="Windows">
    ...
    & "C:\Program Files\Google\Chrome\Application\chrome.exe" `
      --remote-debugging-port=9222 `
      --user-data-dir="$env:USERPROFILE\.lightagent\chrome-cdp"
    ```
  </Tab>
</Tabs>
