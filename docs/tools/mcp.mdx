...
LightAgent supports the [Model Context Protocol (MCP)](https://modelcontextprotocol.io), allowing the Agent to directly invoke tens of thousands of community MCP tools. Configure `mcp.json` once and the tools are exposed to the LLM in exactly the same way as built-in tools — automatically selected and invoked.

## Configuration File

LightAgent reads `~/lightagent/mcp.json`. If the file does not exist, no MCP tools are loaded — and no error is raised.

For Docker deployments, the official `docker-compose.yml` already mounts the host's `./lightagent` directory to `/home/agent/lightagent` inside the container (i.e. the container user's `~/lightagent`). Just drop `mcp.json` into the host's `./lightagent/` directory and it will take effect.
...
LightAgent ships with `read` / `write` / `edit` tools, so **you can simply send the MCP config to the Agent and ask it to write the file**:

For example:

```markdown
Add this MCP to ~/lightagent/mcp.json:
...
- **Hot reload**: When you or the Agent modifies `mcp.json`, changed servers are automatically reloaded after the current message — no need to restart lightagent.
...
| --- | --- |
| Agent has no MCP tools after startup | Verify that `~/lightagent/mcp.json` exists and contains valid JSON |
...
| Docker deployment | Make sure host's `./lightagent` is mounted to `/home/agent/lightagent` in the container, then just drop `mcp.json` into host's `./lightagent/`. Or just ask the Agent to do it |
...
Any MCP server that follows the standard protocol (stdio / SSE / Streamable HTTP) integrates with LightAgent out of the box.
