...
<Warning>
  WeChat group sessions do not participate in global Self-Evolution. They still reuse the Agent reply, tool, and skill pipeline, but they never accumulate global evolution turns, start the review agent, write shared memory, or modify the global workspace. Permanent group memory is generated separately by a `stable_room_id`-scoped group Dream.
</Warning>
