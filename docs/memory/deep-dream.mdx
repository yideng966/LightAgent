...
Deep Dream is the core consolidation mechanism of LightAgent's memory system, responsible for distilling scattered daily memories into refined long-term memory and generating dream diaries.

## Memory Flow

LightAgent's memory progresses through three stages from short-term to long-term:
...
WeChat group sessions never enter the global daily memory, root `MEMORY.md`, or global Deep Dream. Permanent group memory reuses the same two-stage LLM distillation principles while keeping input and storage bound to one `stable_room_id`: new safe archive text is summarized first, then distilled together with that room's active memories.

Both stages use the shared `TextModelRouter.complete()` route. The model cannot select a room ID, and temporary model failures, invalid JSON, or out-of-room evidence do not advance the learning cursor. Group Dream does not extract member profiles.
