...
description: LightAgent command system — Terminal CLI and chat commands
---

LightAgent provides two ways to interact via commands:

- **Terminal CLI** — Run `lightagent <command>` in your system terminal for service management, skill management, and other operations
- **Chat Commands** — Type `/<command>` or `lightagent <command>` in any conversation to check status, manage skills, adjust configuration, etc.

## LightAgent CLI

After deploying with the one-click install script, the `lightagent` command is automatically available. For manual installations, run:

```bash
pip install -e .
```

Then use the `lightagent` command from anywhere:
...
Service:
  start     Start the LightAgent service
  stop      Stop the LightAgent service
  restart   Restart the LightAgent service
...
| Command | Terminal (`lightagent`) | Chat (`/`) |
| --- | :---: | :---: |
| help | ✓ | ✓ |
| version | ✓ | ✓ |
| status | ✓ | ✓ |
