---
title: Skill Management
description: Install, uninstall, enable, disable, and manage skills via commands
---

Skill management commands are used to install, query, and manage LightAgent skills. Use `/skill <subcommand>` in chat or `lightagent skill <subcommand>` in the terminal.

## list

List installed skills and their status.

<CodeGroup>
```text Chat
/skill list
```

```bash Terminal
lightagent skill list
```
</CodeGroup>

Example output:

```
📦 Installed skills (3/4)

✅ pptx
   Use this skill any time a .pptx file is involved…
   Source: skill_hub

✅ skill-creator
   Create, install, or update skills…
   Source: builtin

⏸️ image-vision (disabled)
   Image understanding and visual analysis
   Source: builtin
```

**Browse the Skill Hub** (view all available skills):

<CodeGroup>
```text Chat
/skill list --remote
```

```bash Terminal
lightagent skill list --remote
```
</CodeGroup>

**Options:**

| Option | Description | Default |
| --- | --- | --- |
| `--remote`, `-r` | Browse Skill Hub remote skill list | No |
| `--page` | Page number for remote listing | 1 |

## search

Search for skills on the Skill Hub.

<CodeGroup>
```text Chat
/skill search pptx
```

```bash Terminal
lightagent skill search pptx
```
</CodeGroup>

## install

Install skills with a single `install` command from LightAgent Skill Hub, GitHub, ClawHub, or any URL (zip archives, SKILL.md links) — no manual download or configuration required.

**From Skill Hub (recommended):**

<CodeGroup>
```text Chat
/skill install pptx
```

```bash Terminal
lightagent skill install pptx
```
</CodeGroup>

**From GitHub:**

<CodeGroup>
```text Chat
# Install all skills in a repo (auto-discovers subdirectories with SKILL.md)
/skill install larksuite/cli

# Specify a subdirectory to install a single skill
/skill install https://github.com/larksuite/cli/tree/main/skills/lark-im

# Use # to specify a subdirectory
/skill install larksuite/cli#skills/lark-minutes
```

```bash Terminal
# Install all skills in a repo (auto-discovers subdirectories with SKILL.md)
lightagent skill install larksuite/cli

# Specify a subdirectory to install a single skill
lightagent skill install https://github.com/larksuite/cli/tree/main/skills/lark-im

# Use # to specify a subdirectory
lightagent skill install larksuite/cli#skills/lark-minutes
```
</CodeGroup>

Supports full GitHub URLs and `owner/repo` shorthand. For mono-repos (multiple skills in one repository), omitting the subdirectory auto-discovers and batch-installs all skills; specifying a subdirectory installs only that skill.

**From ClawHub:**

<CodeGroup>
```text Chat
/skill install clawhub:baidu-search
```

```bash Terminal
lightagent skill install clawhub:baidu-search
```
</CodeGroup>

**From URL:**

<CodeGroup>
```text Chat
# Install from a zip archive (single or batch)
/skill install https://cdn.link-ai.tech/skills/pptx.zip

# Install from a SKILL.md link
/skill install https://example.com/path/to/SKILL.md
```

```bash Terminal
# Install from a zip archive (single or batch)
lightagent skill install https://cdn.link-ai.tech/skills/pptx.zip

# Install from a SKILL.md link
lightagent skill install https://example.com/path/to/SKILL.md
```
</CodeGroup>

Supports installing from zip / tar.gz archive URLs — automatically extracts and discovers directories containing `SKILL.md`, with support for single or batch install. Also supports installing directly from a `SKILL.md` file URL, automatically parsing the skill name and description.

## uninstall

Uninstall an installed skill.

<CodeGroup>
```text Chat
/skill uninstall pptx
```

```bash Terminal
lightagent skill uninstall pptx
```
</CodeGroup>

<Warning>
  For official Hub skills, normal uninstall removes package code and its isolated dependency environment while preserving configuration and user data. Add `--purge-data` only when that persistent data should also be deleted.
</Warning>

## outdated / update / rollback / verify

Official Hub skills support explicit lifecycle management:

```bash
lightagent skill outdated
lightagent skill update <name>
lightagent skill update <name> --approve-risk
lightagent skill rollback <name>
lightagent skill verify [name]
```

`outdated` also reports versions marked `yanked` or `revoked`. Updates never run silently. `rollback` restores the previous atomically saved package, and `verify` compares installed files with `skills.lock.json`.

## enable / disable

Enable or disable a skill. Disabled skills will not be invoked by the Agent.

<CodeGroup>
```text Chat
/skill enable pptx
/skill disable pptx
```

```bash Terminal
lightagent skill enable pptx
lightagent skill disable pptx
```
</CodeGroup>

## info

View details of an installed skill, including a preview of its `SKILL.md`.

<CodeGroup>
```text Chat
/skill info pptx
```

```bash Terminal
lightagent skill info pptx
```
</CodeGroup>

## Skill Sources

Installed skills track their origin, viewable via `/skill list`:

| Source | Description |
| --- | --- |
| `builtin` | Built-in project skills |
| `skill_hub` | Installed from LightAgent Skill Hub |
| `github` | Installed directly from a GitHub URL |
| `clawhub` | Installed from ClawHub |
| `url` | Installed from a SKILL.md URL |
| `local` | Locally created skills |
