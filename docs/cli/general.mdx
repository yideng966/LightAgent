...
The following commands can be used in chat with the `/` prefix or in the terminal with the `lightagent` prefix (some are chat-only).
...
Show the current LightAgent version.
