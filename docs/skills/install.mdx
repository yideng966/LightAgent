...
LightAgent supports installing skills from [LightAgent Skill Hub](https://skills.cowagent.ai/), GitHub, ClawHub, LinkAI, and any URL via a unified `install` command. Use `/skill install` in chat or `lightagent skill install` in the terminal.

## From the Skill Hub

Browse all available skills at [Skill Hub](https://skills.cowagent.ai/) and install by name:
