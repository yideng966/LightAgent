...
LightAgent supports installing skills from the signed [LightAgent Skill Hub](https://xiaoguiwucan.github.io/LightAgent-SkillHub/), GitHub, ClawHub, LinkAI, and any URL via a unified `install` command. Use `/skill install` in chat or `lightagent skill install` in the terminal.

## From the Skill Hub

Browse all available skills at [Skill Hub](https://xiaoguiwucan.github.io/LightAgent-SkillHub/) and install by name:
...
/skill outdated              # Detect updates and revoked versions
/skill update pptx           # Explicitly update a skill
/skill rollback pptx         # Restore the previous version
/skill verify pptx           # Verify installed files
```

<Tip>
  All commands above work in the terminal by replacing `/skill` with `lightagent skill`. See [Skill Management Commands](/cli/skill) for full documentation.
</Tip>

Official Hub packages use a signed registry, SHA-256 verification, staging, and atomic replacement. High-risk dependency manifests require an owner or administrator to confirm with `--approve-risk`. Normal uninstall preserves skill configuration and user data; terminal users can add `--purge-data` to explicitly remove them.
