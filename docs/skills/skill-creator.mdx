...
| `metadata.lightagent.requires.bins` | System CLI tools that must be installed |
| `metadata.lightagent.requires.env` | Required environment variables (all must be present) |
| `metadata.lightagent.requires.anyEnv` | Multiple API keys — at least one must be set |
| `metadata.lightagent.requires.anyBins` | Multiple tools — at least one must be installed |
| `metadata.lightagent.always` | Set to `true` to always load, skipping dependency checks |
| `metadata.lightagent.emoji` | Display emoji (optional) |
| `metadata.lightagent.os` | OS restriction, e.g. `["darwin", "linux"]` |
...
## Schema v2 script entrypoints

New or upgraded script skills published to the official Hub must declare `schema_version: 2` and structured `lightagent.entrypoints`. Their instructions call `skill_run` with a skill name, entrypoint name, and string argument array; they must not tell the Agent to launch skill scripts through Bash, Python, or Node command strings. System components use stable `requirements.capabilities` names and are never installed through runtime apt, brew, or sudo commands.
