...
For GPT-5 reasoning models, the Agent Configuration switch is converted to OpenAI's `reasoning_effort`: disabling uses the model's `none` level, while enabled requests use the selected `low`, `medium`, `high`, or `max` value. Support for individual effort values varies by OpenAI model; non-reasoning models receive no reasoning parameter. Third-party compatible endpoints must be configured as a Custom Provider with an explicit thinking protocol instead of inheriting OpenAI behavior automatically.
