...
For Qwen3/QwQ text models, LightAgent converts the global switch to `enable_thinking` and maps `low`, `medium`, `high`, and `max` to increasing thinking budgets. Other Qwen models receive no thinking-specific parameter. Disabling thinking sends `enable_thinking=false` for switchable models.
