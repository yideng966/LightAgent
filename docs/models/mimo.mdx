...
Use the global `enable_thinking` flag to enable or disable model thinking (also switchable from the Web Console settings):
...
MiMo's current switch protocol does not expose LightAgent's four effort levels, so `reasoning_effort` is retained in configuration but no unsupported strength field is sent upstream.
