...
The Agent Configuration page controls Claude text reasoning with the global `enable_thinking` switch and `reasoning_effort` level. When enabled, LightAgent uses adaptive thinking and maps `low`, `medium`, `high`, and `max` to Claude's output effort. When disabled, it sends Claude's explicit disabled thinking mode. This setting affects Agent text requests only and does not change Vision routing.
