...
搜索互联网获取实时信息、新闻、研究等内容。支持博查、百度千帆、智谱、LinkAI、Serper、Jina，配置任意一家即可使用。
...
博查、Serper、Jina 使用 `tools.web_search` 下的专用 Key；百度千帆、智谱、LinkAI 复用对应模型厂商 API Key。
...
- `auto`（默认）：由 Agent 在已配置的厂商中智能选择，并可在一次任务中多次调用、切换不同厂商以获取更全面的结果；未指定时按 `bocha → qianfan → zhipu → linkai → serper → jina` 顺序兜底。
