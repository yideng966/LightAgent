...
LightAgent 支持 [Model Context Protocol (MCP)](https://modelcontextprotocol.io)，让 Agent 能够直接调用社区中数以万计的 MCP 工具。配置一次 `mcp.json`，工具就会以与内置工具完全相同的方式呈现给 LLM，可被自动选择和调用。

## 配置文件

LightAgent 读取 `~/lightagent/mcp.json`。文件不存在时不会启用任何 MCP 工具，也不会报错。

Docker 部署时，官方 `docker-compose.yml` 已经把宿主机 `./lightagent` 挂载到容器内 `/home/agent/lightagent`（即容器用户的 `~/lightagent`），把 `mcp.json` 放进宿主机 `./lightagent/` 目录即可生效。
...
LightAgent 自带 `read` / `write` / `edit` 工具，**直接把要装的 MCP 配置发给 Agent，让它写到配置文件中：

例如：

```markdown
帮我把这个 MCP 加到 ~/lightagent/mcp.json 里：
...
- **热更新**：用户或 Agent 修改 `mcp.json` 后，消息处理完成时会自动重载变更的 server，无需重启 lightagent
...
| 现象 | 排查方向 |
| --- | --- |
| 启动后 Agent 没有 MCP 工具 | 检查 `~/lightagent/mcp.json` 是否存在、JSON 格式是否合法 |
...
| Docker 部署 | 确认宿主机 `./lightagent` 已挂载到容器内 `/home/agent/lightagent`，`mcp.json` 直接放进宿主机 `./lightagent/` 目录即可，或者直接对话 Agent 安装 |
...
只要遵循 MCP 标准协议（stdio / SSE / Streamable HTTP），都可以直接接入 LightAgent。
