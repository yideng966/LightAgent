...
对于 GPT-5 推理模型，「Agent 配置」中的开关会转换为 OpenAI 的 `reasoning_effort`：关闭时使用模型的 `none` 档，开启时使用所选 `low`、`medium`、`high` 或 `max`。不同 OpenAI 模型支持的具体档位可能不同；非推理模型不会收到推理参数。第三方兼容端应配置为 Custom Provider 并显式选择思考协议，不能自动继承 OpenAI 行为。
