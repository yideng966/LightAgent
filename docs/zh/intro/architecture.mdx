---
title: 项目架构
description: LightAgent 2.0 的系统架构和核心设计
---

LightAgent 2.0 从简单的聊天机器人全面升级为超级智能助理，采用 Agent 架构设计，具备自主思考、规划任务、长期记忆和技能扩展等能力。

## 系统架构

LightAgent 的整体架构由以下核心模块组成：

<img src="https://cdn.jsdelivr.net/gh/zhayujie/cowagent-assets@main/architecture/zh/architecture.jpg" alt="LightAgent Architecture" />
...
| **CLI** | 命令行系统，提供终端命令（`lightagent`）和对话命令（`/`），支持进程管理、技能安装、配置修改、知识库管理等操作 |

## Agent 模式

启用 Agent 模式后，LightAgent 会以自主智能体的方式运行，核心工作流如下：
...
Agent 的工作空间默认位于 `~/lightagent` 目录，用于存储系统提示词、记忆文件、技能文件等：
...
秘钥文件单独存储在 `~/.lightagent` 目录（出于安全考虑）：
...
| `agent_workspace` | 工作空间路径 | `~/lightagent` |
...
| `lightagent_lang` | 界面、命令文案、系统提示词等的语言，`auto` 自动检测，可设为 `zh` / `en` | `auto` |
