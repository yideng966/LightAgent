---
title: 功能介绍
description: LightAgent 长期记忆、个人知识库、任务规划、技能系统、CLI 命令、浏览器工具详细说明
---
...
第一次启动 Agent 时，Agent 会主动询问关键信息，并记录至工作空间（默认 `~/lightagent`）中的智能体设定、用户身份、记忆文件中。
...
Agent 会在对话中自动将有价值的信息整理为知识页面，维护交叉引用和索引，通过 Web 控制台可浏览文档和查看知识图谱。知识库存储在工作空间的 `~/lightagent/knowledge/` 目录下。
...
内置 `browser` 工具，Agent 可控制浏览器访问网页、填写表单、点击元素、截图，支持动态 JS 渲染页面。运行 `lightagent install-browser` 一键安装，自动适配服务器（无头模式）和桌面环境：
...
- **自定义技能：** 由用户通过对话创建，存放在工作空间中（`~/lightagent/skills/`），可实现任何复杂的业务流程和第三方系统对接。

安装技能：`/skill install <名称>` 或 `lightagent skill install <名称>`，支持从 Skill Hub、GitHub、ClawHub、URL 等来源安装。
...
访问 [Skill Hub](https://skills.cowagent.ai/) 浏览所有可用技能，或在对话中执行：
...
LightAgent 提供两种命令交互方式，覆盖服务管理、技能安装、配置调整等日常运维操作：

- **终端 CLI：** 在系统终端执行 `lightagent <命令>`，支持 `start`、`stop`、`restart`、`update`、`status`、`logs`、`skill` 等
- **对话命令：** 在对话中输入 `/<命令>`，Web 控制台输入 `/` 可弹出指令菜单快速选择

```bash
lightagent start              # 启动服务
lightagent stop               # 停止服务
lightagent update             # 更新并重启
lightagent skill install pptx # 安装技能
