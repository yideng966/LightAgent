...
**LightAgent** 是一个开源的超级 AI 助理，能够主动思考和规划任务、操作计算机和外部资源、创造和执行 Skills、构建知识库与长期记忆、通过自主进化与你一同成长，是 Agent Harness 工程的最佳实践之一。

LightAgent 轻量、易部署、可扩展，自由接入主流大模型，覆盖微信、飞书、钉钉、企微、QQ、Telegram、Slack、网页等多渠道，7×24 运行于个人电脑或服务器中。
...
<img src="https://cdn.jsdelivr.net/gh/zhayujie/cowagent-assets@main/architecture/zh/architecture.jpg" alt="LightAgent Architecture" width="750"/>

LightAgent 是一个完整的 **Agent Harness**：消息从各类**通道**进入，**Agent Core** 结合记忆、知识库与可用工具/技能进行任务规划与决策，调用**模型**生成结果，再回传至原通道。各模块解耦清晰，按需扩展。
...
安装后可使用 `lightagent` [CLI 命令](https://docs.cowagent.ai/zh/cli) 管理服务：

```bash
lightagent start | stop | restart        # 服务管理
lightagent status | logs                  # 状态和日志
lightagent update                         # 拉取最新代码并重启
lightagent skill install <名称>           # 安装技能
lightagent install-browser                # 安装浏览器工具
```
...
LightAgent 支持国内外主流厂商的大语言模型。**文本对话、图像理解、图像生成、语音识别/合成、向量** 等能力均可独立配置厂商。
...
<img src="https://cdn.jsdelivr.net/gh/zhayujie/cowagent-assets@main/screenshots/zh/web-console-chat.png" alt="LightAgent Web 控制台" width="800"/>
...
**长期记忆**采用三层架构：对话上下文（短期）→ 天级记忆（中期）→ MEMORY.md（长期）。每日自动执行**梦境蒸馏（Deep Dream）**，将分散记忆整合为精炼的长期记忆并生成叙事日记。微信群的永久记忆同样属于记忆系统，按稳定群 ID 隔离，可在 Web 控制台「管理 → 记忆 → 群记忆」中查询、维护和查看生成状态。详见 [长期记忆](https://docs.cowagent.ai/zh/memory) · [梦境蒸馏](https://docs.cowagent.ai/zh/memory/deep-dream)。

**个人知识库** 与按时间记录的记忆不同，以**主题为维度**组织结构化知识。Agent 在对话中自动整理有价值信息，维护交叉引用与索引，Web 控制台「管理 → 知识」只管理全局 Markdown 知识库与知识图谱。详见 [个人知识库](https://docs.cowagent.ai/zh/knowledge)。
...
> **2026.04.01：** [v2.0.5](https://github.com/zhayujie/CowAgent/releases/tag/2.0.5) — LightAgent CLI 命令系统、Skill Hub 开源、浏览器工具、企微扫码创建
...
- 🐛 [提交 Issue](https://github.com/yideng966/LightAgent/issues)
...
- **[LightAgent Skill Hub](https://github.com/zhayujie/cow-skill-hub)** — 开源的 AI Agent 技能广场，支持 LightAgent、OpenClaw、Claude Code 等多种 Agent
...
> [LinkAI](https://link-ai.tech/) 是面向企业和个人的一站式 AI 智能体平台，为 LightAgent 提供云端托管和企业级支持：
>
> - **🚀 免部署在线运行**：无需服务器即可创建 [LightAgent 在线助理](https://link-ai.tech/cowagent/create)，1 分钟拥有专属 Agent
...
3. LightAgent 项目专注于开源技术开发，不会参与、授权或发行任何加密货币。

<br/>

## 📌 项目更名说明

本项目原名 `chatgpt-on-wechat`，于 2026.04.13 正式更名为 **LightAgent**。原 GitHub 地址已自动重定向，老用户可选择执行 `git remote set-url origin https://github.com/yideng966/LightAgent.git` 更新本地远程地址。
