...
技能管理命令用于安装、查询和管理 LightAgent 的技能。在对话中使用 `/skill <子命令>`，在终端中使用 `lightagent skill <子命令>`。
...
安装技能。通过统一的 `install` 命令，可一键安装来自 **LightAgent 技能广场、GitHub、ClawHub** 以及任意 URL（zip 压缩包、SKILL.md 链接）上的技能，无需手动下载和配置。

**从 LightAgent 技能广场安装（推荐）：**
...
| --- | --- |
| `builtin` | 项目内置技能 |
| `skill_hub` | 从 LightAgent Skill Hub 安装 |
