---
title: Web 控制台
description: 通过 Web 控制台使用 LightAgent
---

Web 控制台是 LightAgent 的默认通道，启动后会自动运行，通过浏览器即可与 Agent 对话，并支持在线管理模型、技能、记忆、通道等配置。
...
微信群自由回复启用 LLM Scorer 时，请在「模型管理 -> LLM Scorer」能力卡中单独选择 Provider 和模型。该选择复用模型管理中的厂商凭据，不会修改主模型；自由回复页面仅保留开关、上下文长度、判定阈值、Token 上限和失败回落等行为设置。OpenAI-compatible Provider 使用原生 JSON Mode，其他 Provider 使用严格 JSON Prompt、解析校验与失败回落。删除自定义 Provider 前，需要先将 LLM Scorer 切换到其他 Provider。

「群聊 -> 自由回复」中的“允许过期消息数”控制观察式回复生成期间可以容忍的同群新增消息，默认 `5`、范围 `0–100`；设为 `0` 时，只要出现新群消息就抑制该回复。期间若已经产生新的机器人回复，旧候选仍会被直接抑制，避免重复或乱序接话。
