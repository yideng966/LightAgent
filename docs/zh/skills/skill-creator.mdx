...
| `metadata.lightagent.requires.bins` | 系统里必须装了哪些命令行工具 |
| `metadata.lightagent.requires.env` | 需要哪些环境变量（全部满足才行） |
| `metadata.lightagent.requires.anyEnv` | 多个 API Key 满足一个就行 |
| `metadata.lightagent.requires.anyBins` | 多个工具满足一个就行 |
| `metadata.lightagent.always` | 设为 `true` 会始终加载，不检查依赖 |
| `metadata.lightagent.emoji` | 展示用的 emoji（可选） |
| `metadata.lightagent.os` | 限定系统，如 `["darwin", "linux"]` |
...
## Skill Hub Schema v2 脚本入口

向官方 Hub 新发布或升级的脚本技能必须声明 `schema_version: 2` 和结构化 `lightagent.entrypoints`。正文说明调用 `skill_run` 的技能名、入口名和字符串参数数组，不要让 Agent 通过 Bash、Python 或 Node 命令字符串直接执行技能脚本。

```yaml
schema_version: 2
requirements:
  capabilities: [office-documents]
lightagent:
  tools: [skill_run]
  entrypoints:
    - name: generate
...
`requirements.capabilities` 只允许稳定能力名；技能不得包含运行时 `apt`、`brew` 或 `sudo` 安装步骤。
