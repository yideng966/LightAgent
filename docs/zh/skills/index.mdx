...
LightAgent 提供多种方式获取技能：

- [LightAgent 技能广场](https://skills.cowagent.ai/) — 在线浏览所有可用技能，或通过 `/skill list --remote` 在对话中浏览和安装
...
1. **项目内置技能**：直接从项目 `skills/` 目录加载，名称受保护。
2. **工作空间技能**：从 `~/lightagent/skills/` 加载用户创建或安装的非同名技能。

工作空间技能不能覆盖项目内置同名技能。升级遗留的同名工作空间副本会被忽略但不会自动删除；通过 Skill Hub、GitHub、GitLab、Git URL、ZIP、URL、本地目录、Cloud 或对话创建自定义版本时，必须使用新名称。
