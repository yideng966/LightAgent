---
title: Telegram
description: Integrate LightAgent with Telegram via the Bot API
---

> Integrate LightAgent into Telegram via the official Bot API. Supports private chat and group chat (triggered by @mention or replying to the bot). Uses Long Polling — no public IP required, works out of the box.
...
2. Send `/newbot` and follow the prompts:
   - **Bot name** (display name, e.g. `My LightAgent Bot`)
   - **Bot username** (must end with `bot`, e.g. `my_lightagent_bot`)
...
```
[Telegram] Bot logged in as @my_lightagent_bot (id=123456789)
...
- **Private chat**: search for your bot username (e.g. `@my_lightagent_bot`) in Telegram, click `Start` and chat away.
