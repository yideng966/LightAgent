---
title: Web Console
description: Use LightAgent through the Web Console
---

The Web Console is LightAgent's default channel. It runs automatically once started, letting you chat with the Agent in a browser and manage models, skills, memory, channels, and other configuration online.
...
| `reasoning_effort` | Unified reasoning level used while thinking is enabled: `low`, `medium`, `high`, or `max` | `low` |
...
Supports streaming output with real-time display of the Agent's reasoning process and tool calls. Under **Configuration -> Agent Configuration**, the deep-thinking switch controls whether Agent text requests ask for reasoning, and the adjacent selector sets `low`, `medium`, `high`, or `max`. Disabling thinking keeps the selected level for the next time it is enabled. Providers translate the unified level to their own protocol; models with fewer levels may merge adjacent values, and fixed-reasoning models may not support being turned off.

On mobile, the left navigation is collapsed by default and opens from the menu button at the top left. The console uses the dynamic viewport height and avoids focusing the bottom composer immediately after login, keeping the navigation entry visible in Android browsers.
...
The Main Model, its fallback rows, LLM Scorer, Image Understanding, Image Generation, Speech Recognition, Speech Synthesis, and Embedding cards accept direct model-ID entry. You can also click **Fetch models** beside a field to load candidates from the selected configured provider's Models API. The editable list supports Chinese and English keyword filtering. Within the current page session, a successfully fetched provider catalog is reused automatically by other capabilities and fallback rows using that provider. If the provider does not expose a model-list endpoint or the request fails, the current value is preserved and can still be entered and saved manually.
...
When editing a Personal WeChat Groups task, the current target group is shown and can be changed using a dropdown of stable groups selected in the channel settings. The task's current target remains visible while temporarily offline.
