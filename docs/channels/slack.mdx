---
title: Slack
description: Integrate LightAgent with a Slack App
---

> Integrate LightAgent into Slack via a Slack App in **Socket Mode**. Supports direct messages (DM) and channels (triggered by @mention or replying within a thread). Socket Mode uses a persistent WebSocket connection — no public IP or callback URL required, works out of the box.
...
2. Enter an **App Name** (e.g. `LightAgent`), pick the **Workspace** to install into, and create it.
