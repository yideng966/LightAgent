---
title: Architecture
description: LightAgent 2.0 system architecture and core design
---

LightAgent 2.0 has evolved from a simple chatbot into a super intelligent assistant with Agent architecture, featuring autonomous thinking, task planning, long-term memory, and skill extensibility.

## System Architecture

LightAgent's architecture consists of the following core modules:

<img src="https://cdn.jsdelivr.net/gh/zhayujie/cowagent-assets@main/architecture/en/architecture.png" alt="LightAgent Architecture" />
...
| **CLI** | Command-line system providing terminal commands (`lightagent`) and chat commands (`/`) for process management, skill installation, configuration, knowledge base management, and more |

## Agent Mode Workflow

When Agent mode is enabled, LightAgent runs as an autonomous agent with the following workflow:
...
The Agent workspace is located at `~/lightagent` by default and stores system prompts, memory files, and skill files:
...
Secret keys are stored separately in `~/.lightagent` directory for security:
...
| `agent_workspace` | Workspace path | `~/lightagent` |
