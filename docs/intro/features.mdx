---
title: Features
description: LightAgent long-term memory, task planning, skills system, CLI commands, and browser tool in detail
---
...
On first launch, the Agent proactively asks the user for key information and records it in the workspace (default `~/lightagent`) — including agent settings, user identity, and memory files.
...
The Agent automatically organizes valuable information from conversations into knowledge pages, maintaining cross-references and indexes. The Web console provides document browsing and knowledge graph visualization. Knowledge is stored in `~/lightagent/knowledge/` within the workspace.
...
- **Custom skills:** Created by users through conversation, stored in the workspace (`~/lightagent/skills/`), capable of implementing any complex business process or third-party integration.

Install skills: `/skill install <name>` or `lightagent skill install <name>`, supporting Skill Hub, GitHub, ClawHub, URL, and more.
...
Visit [Skill Hub](https://skills.cowagent.ai/) to browse all available skills, or use commands in conversation:
...
LightAgent provides two command interaction methods, covering service management, skill installation, configuration, and more:

- **Terminal CLI:** Run `lightagent <command>` in the system terminal, supporting `start`, `stop`, `restart`, `update`, `status`, `logs`, `skill`, etc.
...
lightagent update             # Update and restart
lightagent skill install pptx # Install a skill
lightagent install-browser    # Install browser tool
```
