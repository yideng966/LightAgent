...
from voice.factory import UnsupportedVoiceProviderError, create_voice
...
class Bridge(object):
        ...
        if self.bots.get(typename) is None:
            ...
            if typename == "text_to_voice":
                self.bots[typename] = create_voice(
                    self.btype[typename], capability=typename
                )
    ...
    def fetch_voice_to_text(self, voiceFile) -> Reply:
        try:
            ...
            logger.error(
                f"[Bridge] unsupported voice provider: "
                f"capability=voice_to_text, provider={exc.voice_type}"
            )
            return Reply(
                ReplyType.ERROR,
                f"Unsupported voice_to_text provider: {exc.voice_type}",
            )
    ...
    def fetch_text_to_voice(self, text) -> Reply:
        try:
            ...
            logger.error(
                f"[Bridge] unsupported voice provider: "
                f"capability=text_to_voice, provider={exc.voice_type}"
            )
            return Reply(
                ReplyType.ERROR,
                f"Unsupported text_to_voice provider: {exc.voice_type}",
            )
