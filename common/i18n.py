...
Resolution priority (highest first):
  1. Explicit `lightagent_lang` from config.json — also covers Docker/CI, since any
     config key is overridable via its uppercase env var (e.g. LIGHTAGENT_LANG=zh),
     handled by config.load_config() before resolution. LIGHTAGENT_LANG is a private
...
def _detect_from_env():
    ...
    The lightagent_lang env override (LIGHTAGENT_LANG=zh) is intentionally NOT read here:
    it sets config["lightagent_lang"] and is handled via the explicit config path,
    not auto-detection.
    """
...
def detect_language():
    ...
    Order (auto-detection only; explicit config["lightagent_lang"] is resolved
...
def resolve_language(configured=None):
    ...
    `configured` is the raw `lightagent_lang` value from config.json (may be None,
