...
class CloudClient(LinkAIClient):
    ...
    def on_chat(self, data: dict, send_chunk_fn):
        ...
        # Intercept lightagent/slash commands before the agent runs
        try:
            from plugins import PluginManager
            mgr = PluginManager()
            instance = mgr.instances.get("LIGHTAGENT_CLI")
            ...
            logger.warning(f"[CloudClient] lightagent_cli intercept failed: {e}")
...
LIGHTAGENT_SEND_WEB_SUBDIR = "lightagent-send"


def copy_send_file(src_path: str, workspace_root: str) -> str:
    """Copy *src_path* into ``websites/lightagent-send/`` and return its URL.
    ...
    send_dir = os.path.join(ws, "websites", LIGHTAGENT_SEND_WEB_SUBDIR)
    try:
    ...
    try:
        ...
        return ""
    return f"{base}/{LIGHTAGENT_SEND_WEB_SUBDIR}/{dest_name}"
