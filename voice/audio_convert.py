...
_SILK_EXTENSIONS = (".sil", ".silk", ".slk")
_AUDIO_FORMAT_EXTENSIONS = {
    "wav": "wav",
    "silk": "sil",
    "mp3": "mp3",
    "amr": "amr",
...
def _require_pysilk():
    if pysilk is None:
        raise ImportError(
            'pysilk-mod>=1.6.4 is required for Silk audio conversion. '
            'Install it with: python -m pip install "pysilk-mod>=1.6.4"'
        )
    return pysilk


def sniff_audio_format(path):
    try:
...
def audio_extension_for_format(audio_format):
    return _AUDIO_FORMAT_EXTENSIONS.get(str(audio_format or "").lower(), "")
...
def any_to_wav(any_path, wav_path):
    ...
    把任意格式转成wav文件
    """
    normalized_path = str(any_path).lower()
    detected_format = sniff_audio_format(any_path)
    if normalized_path.endswith(".wav") or detected_format == "wav":
        shutil.copy2(any_path, wav_path)
        return
    if normalized_path.endswith(_SILK_EXTENSIONS) and detected_format in ("", "silk"):
...
def any_to_sil(any_path, sil_path):
    ...
    silk_data = _require_pysilk().encode(wav_data, data_rate=rate, sample_rate=rate)
...
def sil_to_wav(silk_path, wav_path, rate: int = 24000):
    ...
    silk 文件转 wav
    """
    wav_data = _require_pysilk().decode_file(silk_path, to_wav=True, sample_rate=rate)
