...
pip install pydub
pip install "pysilk-mod>=1.6.4"
还有ffmpeg，不同系统安装方式不同

`pysilk-mod` 是发行包名，Python 导入名仍为 `pysilk`。Windows 需要与 Python 版本和架构匹配的 wheel，并安装对应的 Microsoft Visual C++ Runtime。
安装后可执行 `python -c "import pysilk; print(pysilk.__version__)"` 验收导入。
