...
# loaded in a background thread (so command plugins like lightagent_cli/godcmd work
        ...
        with self._lock:
            ...
            if first_start:
                if DESKTOP_MODE:
                    ...
                    # (lightagent_cli / godcmd, e.g. /status, #help) work in the
...
def _stop_wechat_group_channel(channel_manager):
    if channel_manager is None:
        return
    try:
        if channel_manager.get_channel(const.WECHAT_GROUP) is None:
            return
        channel_manager.stop(const.WECHAT_GROUP)
    except Exception as e:
        logger.warning(f"[App] Failed to stop WeChat group channel during shutdown: {e}")
...
def run():
    ...
    try:
        ...
        _stop_wechat_group_channel(_channel_mgr)
