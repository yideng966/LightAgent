...
- 发行说明会通过 GitHub Release 链接卡片在微信群、通知渠道等场景中预览展示，第一段摘要和所有条目必须是群成员能一眼看懂的自然语言。**严禁出现任何内部实现细节**，包括但不限于：函数名、类名、模块名、文件路径、配置键名、正则表达式、标记常量、内部命中原因（如 `current_image`、`ambiguous_recent_images`）、诊断字段名（如 `summary_generated`）、模型名称（如 `agnes-2.0-flash`）或验证记录中的测试数量。用”修复了什么现象””现在表现如何”替代”改了哪个函数””增加了什么标记”。技术细节只保留在 `CHANGES.md` 中。
...
- 普通群成员可以问答、查询、总结和读取上下文，但不能触发知识库写入、永久记忆写入、群记忆写入、群友画像写入、自主进化、技能新增/修改/删除、workspace 文件写入/编辑、定时任务修改或微信群配置修改。
