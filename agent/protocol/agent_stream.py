...
def _summarize_user_message_for_run_log(user_message: str) -> str:
    ...
    for tag, label in (
        ("wechat-group-mention-verification", "mention_verification"),
        ("wechat-group-reply-policy", "reply_policy"),
        ("wechat-group-archive-evidence", "archive_evidence"),
        ("local-extractive-summary", "local_summary"),
        ("wechat-group-focus", "focus"),
        ("wechat-group-style", "style"),
        ("wechat-group-emotion", "emotion"),
        ("wechat-group-reference-policy", "reference_policy"),
        ("wechat-group-multimodal", "multimodal"),
    ):
        if _extract_tag_block(message, tag):
...
class AgentStreamExecutor:
    ...
    def _check_consecutive_failures(self, tool_name: str, args: dict) -> Tuple[bool, str, bool]:
        ...
        # Hard stop at 8 failures. Retrieval tools are non-destructive and may
        # have already collected useful context, so return a recoverable tool
        # error instead of aborting the whole agent run.
        if same_tool_failures >= 8:
            if tool_name in RECOVERABLE_RETRIEVAL_TOOLS:
                return True, _t(
                    f"检索工具 '{tool_name}' 已连续失败 {same_tool_failures} 次。停止继续抓取或猜测链接，"
                    "请基于已经成功获取的内容总结；如果资料不足，请向用户询问更准确的链接。",
                    f"Retrieval tool '{tool_name}' failed {same_tool_failures} times in a row. Stop fetching "
                    "or guessing links, summarize from the content already retrieved; if it is insufficient, "
                    "ask the user for a more accurate link.",
                ), False
            return True, _t(
        ...
        try:
            ...
            for chunk in stream:
                ...
                if isinstance(chunk, dict) and chunk.get("error"):
                    fallback_exhausted = bool(chunk.get("model_fallback_exhausted"))
                    ...
                    if is_overflow:
                        ...
                        model_error.model_fallback_exhausted = fallback_exhausted
                        raise model_error
            ...
            fallback_exhausted = bool(getattr(e, "model_fallback_exhausted", False))
            is_retryable = not fallback_exhausted and any(keyword in error_str_lower for keyword in [
