...
MAX_REDIRECTS = 10
MAX_RECOVERY_PARENT_DEPTH = 2
MAX_RECOVERY_CANDIDATES = 5
MAX_RECOVERY_CANDIDATE_PROBES = 5
...
class _AnchorHrefParser(HTMLParser):
    """Collect href values from real anchor tags."""
...
class WebFetch(BaseTool):
    ...
    def _fetch_webpage(self, url: str) -> ToolResult:
        ...
        try:
            ...
            if status_code in (404, 410):
                return ToolResult.fail(self._build_not_found_recovery_error(url, status_code))
            return ToolResult.fail(f"Error: HTTP {status_code} for URL: {url}")
        except ValueError as e:
            return ToolResult.fail(f"Error: {e}")
    ...
    def _build_not_found_recovery_error(self, url: str, status_code: int) -> str:
        """Build a bounded same-site recovery hint for 404/410 documentation URLs."""
        base_error = f"Error: HTTP {status_code} for URL: {url}"
        parent_urls = self._candidate_parent_urls(url)
        if not parent_urls:
            return base_error + "\nNo parent paths are available. Do not keep guessing deeper URLs."

        lines = [
            base_error,
            "Recovery: the requested page was not found. Try parent paths:",
        ]
        lines.extend(f"- {parent_url}" for parent_url in parent_urls)

        for parent_url in parent_urls:
            try:
                validate_url_safe(parent_url)
                response = self._safe_get(parent_url)
                response.raise_for_status()
            except Exception:
                continue

            response.encoding = self._detect_encoding(response)
            candidates = self._extract_same_site_navigation_candidates(url, parent_url, response.text)
            if candidates:
                lines.append(f"Navigation candidates from {parent_url}:")
                lines.extend(f"- {candidate}" for candidate in candidates)
                lines.append("Do not keep guessing deeper URLs.")
                return "\n".join(lines)

        lines.append("No reachable same-site parent page was found. Do not keep guessing deeper URLs.")
        return "\n".join(lines)

    @staticmethod
    def _candidate_parent_urls(url: str) -> list:
        """Return nearest parent URLs with a small depth cap."""
        parsed = urlparse(url)
        path = parsed.path or "/"
        ...
        for _ in range(MAX_RECOVERY_PARENT_DEPTH):
            parent_path = os.path.dirname(stripped) if stripped else ""
            if not parent_path or parent_path == ".":
                parent_path = "/"
            if not parent_path.endswith("/"):
                parent_path += "/"
            parent_url = parsed._replace(path=parent_path, params="", query="", fragment="").geturl()
            ...
            if parent_path == "/":
                break
            stripped = parent_path.rstrip("/")

        return parents

    def _extract_same_site_navigation_candidates(self, original_url: str, parent_url: str, html: str) -> list:
        """Extract bounded, reachable same-origin http(s) links from a parent page."""
        ...
        for href in parser.hrefs:
            if probes >= MAX_RECOVERY_CANDIDATE_PROBES:
                break
            candidate = urljoin(parent_url, href)
            parsed = urlparse(candidate)
            if parsed.scheme not in ("http", "https"):
                continue
            if parsed.scheme.lower() != original.scheme.lower():
                continue
            if parsed.netloc.lower() != original.netloc.lower():
                continue
            ...
            try:
                ...
                candidate_response = self._safe_get(candidate)
