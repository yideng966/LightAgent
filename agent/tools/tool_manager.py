...
class ToolManager:
    ...
    def _mcp_json_path(self) -> str:
        import os
        workspace = os.path.expanduser(conf().get("agent_workspace", "~/lightagent"))
        return os.path.join(workspace, "mcp.json")

    def _read_mcp_json_signature(self):
        """
        Return (mtime, sha256_of_bytes) for ~/lightagent/mcp.json without parsing.
    ...
    def _load_mcp_configs(self) -> list:
        ...
        Load MCP server configs with priority:
          1. ~/lightagent/mcp.json  (supports both mcpServers and mcp_servers keys)
    ...
    def refresh_mcp_if_changed(self):
        """
        Cheap check whether ~/lightagent/mcp.json has changed since last load.
    ...
    def sync_mcp_into_agent(self, agent) -> tuple:
        ...
        Handles both representations LightAgent uses:
