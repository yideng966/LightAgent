...
_DEFAULT_USER_DATA_DIR = "~/.lightagent/browser_profile"
...
() => {
    ...
    window.__lightAgentRefMap = refMap;
        ...
        try:
            ...
            if "singletonlock" in msg or "profile" in msg or "lock" in msg:
                raise RuntimeError(
                    ...
                    "Close the other Chromium / lightagent instance, or set a different "
        ...
        try:
            if ref is not None:
                result = page.evaluate(f"""
                    () => {{
                        const el = window.__lightAgentRefMap && window.__lightAgentRefMap[{ref}];
