...
class Vision(BaseTool):
    ...
    def _call_api(self, provider: VisionProvider, model: str,
        ...
        payload = {
            "model": model,
            # Some OpenAI-compatible gateways (new-api / sub2api etc.) default to
            # SSE when stream is omitted; Vision always needs a full JSON body.
            "stream": False,
            "messages": [
                {
                    "role": "user",
        ...
        try:
            ...
            raise VisionAPIError(
                f"Invalid JSON response (content-type={content_type!r}): "
                f"{body_preview or '<empty body>'} ({e})"
            )
        ...
        if "error" in data:
            ...
            if isinstance(err, dict):
                ...
                msg = str(err) or "Unknown API error"
