...
Agent: [调用 scheduler 工具]
      ...
      tool_call:
        tool_name: read
        tool_params:
          file_path: ~/lightagent/schedule.txt
        result_prefix: 📅 今日日程
      schedule_type: cron
      schedule_value: 0 8 * * *
```
...
任务保存在 JSON 文件中：
```
~/lightagent/scheduler/tasks.json
```
...
Agent: ✅ 定时任务创建成功
       任务ID: schedule001
       调度: 每天 8:00
       工具: read(file_path='~/lightagent/schedule.txt')
       前缀: 📅 今日日程
```
...
用户: 每天晚上11点备份工作文件
Agent: [创建 cron: 0 23 * * *]
       工具: bash(command='cp ~/lightagent/work.txt ~/lightagent/backup/work_$(date +%Y%m%d).txt')
       前缀: ✅ 文件已备份
```
