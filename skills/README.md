...
Browse, search, and install skills from [LightAgent Skill Hub](https://skills.cowagent.ai/).

Open source: [github.com/zhayujie/lightagent-skill-hub](https://github.com/zhayujie/cow-skill-hub)

## Install Skills

Install skills from multiple sources via chat (`/skill`) or terminal (`lightagent skill`):
...
> In terminal, replace `/skill` with `lightagent skill`.
...
metadata: {"lightagent":{"emoji":"🔧","requires":{"bins":["tool"],"env":["API_KEY"]}}}
---
...
| `metadata.lightagent.emoji` | Display emoji |
| `metadata.lightagent.always` | Always include this skill (default: false) |
| `metadata.lightagent.requires.bins` | Required binaries |
| `metadata.lightagent.requires.env` | Required environment variables |
| `metadata.lightagent.requires.config` | Required config paths |
| `metadata.lightagent.os` | Supported OS (e.g., `["darwin", "linux"]`) |
...
2. **Custom skills** (higher): `~/lightagent/skills/` — installed via `lightagent skill install` or skill creator
