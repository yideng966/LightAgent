...
Usage:
    ...
    python generate.py --json-file <path>
...
_IMAGE_DOWNLOAD_PROXY = ""
_IMAGE_DOWNLOAD_PROXY_DOMAINS: list[str] = []
...
def _normalize_domain_patterns(value) -> list[str]:
    ...
    for item in raw:
        ...
        if text.startswith("*."):
            host = text[2:].strip(".")
            normalized = f"*.{host}" if host else ""
        else:
...
def _should_use_proxy(source: str, proxy: str, proxy_domains) -> bool:
    if not proxy or os.path.isfile(source):
        return False
    domains = _normalize_domain_patterns(proxy_domains)
    if not domains:
        return False
    host = (urlparse(source).hostname or "").strip().lower().strip(".")
    return any(_domain_matches(host, pattern) for pattern in domains)
...
def _set_image_download_proxy(proxy: str, proxy_domains) -> None:
    global _IMAGE_DOWNLOAD_PROXY, _IMAGE_DOWNLOAD_PROXY_DOMAINS
    _IMAGE_DOWNLOAD_PROXY = _normalize_proxy_url(proxy)
    _IMAGE_DOWNLOAD_PROXY_DOMAINS = _normalize_domain_patterns(proxy_domains)


def _load_image(source: str, *, proxy: str | None = None, proxy_domains=None) -> bytes:
    """Load image from a local file path or URL."""
    if os.path.isfile(source):
        with open(source, "rb") as f:
            return f.read()
    active_proxy = _IMAGE_DOWNLOAD_PROXY if proxy is None else _normalize_proxy_url(proxy)
    active_domains = _IMAGE_DOWNLOAD_PROXY_DOMAINS if proxy_domains is None else _normalize_domain_patterns(proxy_domains)
    use_proxy = _should_use_proxy(source, active_proxy, active_domains)
    if _HAS_REQUESTS:
        kwargs = {"timeout": 60}
        if use_proxy:
            kwargs["proxies"] = _proxy_dict(active_proxy)
        resp = requests.get(source, **kwargs)
        resp.raise_for_status()
        return resp.content
    req = Request(source)
    if use_proxy:
        opener = build_opener(ProxyHandler(_proxy_dict(active_proxy)))
...
def _merged_namespace(data: dict, legacy: str, canonical: str) -> dict:
    legacy_section = data.get(legacy)
    canonical_section = data.get(canonical)
    merged = {}
    if isinstance(legacy_section, dict):
        _deep_merge_dicts(merged, legacy_section)
    if isinstance(canonical_section, dict):
        _deep_merge_dicts(merged, canonical_section)
    return merged


def _load_image_proxy_config_from_config_file() -> dict:
    _ensure_project_root_on_path()
    from config import get_data_root
    ...
    if not isinstance(data, dict):
        return {}
    tools_cfg = _merged_namespace(data, legacy="tool", canonical="tools")
    web_fetch_cfg = tools_cfg.get("web_fetch") or {}
    if not isinstance(web_fetch_cfg, dict):
        web_fetch_cfg = {}
    skills_cfg = _merged_namespace(data, legacy="skill", canonical="skills")
    image_cfg = skills_cfg.get("image-generation") or {}
    if not isinstance(image_cfg, dict):
        image_cfg = {}
    return {
        "proxy": web_fetch_cfg.get("proxy") or "",
        "proxy_enabled": image_cfg.get("proxy_enabled", False),
...
def _load_cli_args(argv: list[str]) -> dict:
    if not argv:
        raise ValueError("Usage: python generate.py '<json_args>' or --json-file <path>")

    if argv[0] == "--json-file":
        if len(argv) != 2:
            raise ValueError("--json-file requires exactly one file path")
        try:
            raw = Path(argv[1]).expanduser().read_text(encoding="utf-8-sig")
        except OSError as e:
            raise ValueError(f"Unable to read JSON file: {e}") from e
    else:
        if len(argv) != 1:
            raise ValueError(
                "Expected one JSON argument. On Windows, use --json-file <path> "
                "instead of single-quoted inline JSON."
            )
        raw = argv[0]
...
def main():
    try:
        args = _load_cli_args(sys.argv[1:])
    ...
    file_proxy_cfg = _load_image_proxy_config_from_config_file()
    proxy = args.get("proxy") or os.environ.get("SKILL_IMAGE_GENERATION_PROXY") or file_proxy_cfg.get("proxy") or ""
    proxy_enabled = _as_bool(
        args.get("proxy_enabled"),
        _as_bool(os.environ.get("SKILL_IMAGE_GENERATION_PROXY_ENABLED"), _as_bool(file_proxy_cfg.get("proxy_enabled"), False)),
    )
    if "proxy_domains" in args:
        proxy_domains = args.get("proxy_domains")
    elif os.environ.get("SKILL_IMAGE_GENERATION_PROXY_DOMAINS") is not None:
        proxy_domains = os.environ.get("SKILL_IMAGE_GENERATION_PROXY_DOMAINS")
    else:
        proxy_domains = file_proxy_cfg.get("proxy_domains") or []
    if proxy_enabled:
        _set_image_download_proxy(proxy, proxy_domains)
