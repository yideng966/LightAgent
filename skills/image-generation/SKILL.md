...
Run `scripts/generate.py` with a UTF-8 JSON request file. The path is relative to this skill's `base_dir`.

1. Use the `write` tool to create a uniquely named request file under the workspace `tmp/` directory. Do not create it with shell `echo` because prompts can contain shell metacharacters.
2. Run the script with that file:

```bash
python "<base_dir>/scripts/generate.py" --json-file "<workspace>/tmp/image-generation-request-<unique>.json"
```

On Windows, the `bash` tool runs through `cmd.exe`: **never pass inline JSON wrapped in single quotes**. `cmd.exe` does not treat single quotes as argument delimiters and will corrupt the JSON. The `--json-file` form works on every supported platform and is the preferred form. Inline JSON remains supported on POSIX shells for backward compatibility only.
...
Write the request file with this content:
...
Then call `generate.py --json-file <request_file>` as shown above.
...
Write the selected JSON to a unique request file, then call `generate.py --json-file <request_file>`.
