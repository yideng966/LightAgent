...
# Clone project
clone_project() {
    echo -e "${GREEN}🔍 Cloning LightAgent project...${NC}"

    if [ -d "LightAgent" ]; then
        # An existing directory is automatically backed up (no prompt) so the
        # installer stays one-shot / hands-off.
        local backup_dir="LightAgent_backup_$(date +%s)"
        echo -e "${YELLOW}⚠️  $(t "目录 'LightAgent' 已存在，自动备份到" "Directory 'LightAgent' exists, backing up to") '$backup_dir'...${NC}"
        mv LightAgent "$backup_dir"
    fi
    ...
    if ! command -v git &> /dev/null; then
        ...
        local zip_url="https://github.com/yideng966/LightAgent/archive/refs/heads/master.zip"
        if command -v wget &> /dev/null; then
            wget "$zip_url" -O LightAgent.zip
        elif command -v curl &> /dev/null; then
            curl -L "$zip_url" -o LightAgent.zip
        else
        ...
        if command -v unzip &> /dev/null; then
            unzip LightAgent.zip
        elif command -v python3 &> /dev/null; then
            python3 -m zipfile -e LightAgent.zip .
        elif command -v python &> /dev/null; then
            python -m zipfile -e LightAgent.zip .
        else
            echo -e "${RED}❌ Cannot extract archive. Please install 'unzip' or Python.${NC}"
            exit 1
        fi
        # Archive top-level dir name may vary (LightAgent-master, etc.); detect it.
        local _extracted="LightAgent-master"
        if [ ! -d "$_extracted" ]; then
            _extracted=$(ls -d LightAgent-*/ 2>/dev/null | head -1 | sed 's:/*$::')
        fi
        [ -n "$_extracted" ] && [ -d "$_extracted" ] && mv "$_extracted" LightAgent
        rm -f LightAgent.zip
    else
        ...
        if curl -sI --connect-timeout 5 --max-time 10 https://github.com > /dev/null 2>&1; then
            ...
            _timeout 60 git clone --depth 10 --progress https://github.com/yideng966/LightAgent.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
            echo -e "${YELLOW}⚠️  GitHub clone failed or timed out, retrying...${NC}"
            _timeout 30 git clone --depth 10 --progress https://github.com/yideng966/LightAgent.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
    ...
    cd LightAgent || { echo -e "${RED}❌ Failed to enter project directory.${NC}"; exit 1; }
    export BASE_DIR=$(pwd)
...
install_dependencies() {
    ...
    # Register `lightagent` CLI command via editable install
    echo -e "${YELLOW}Registering lightagent CLI...${NC}"
    set +e
    $PYTHON_CMD -m pip install -e . $PIP_EXTRA_ARGS $PIP_MIRROR > /dev/null 2>&1
    if command -v lightagent &> /dev/null; then
        echo -e "${GREEN}✅ lightagent CLI registered.${NC}"
    else
        echo -e "${YELLOW}⚠️  lightagent CLI not in PATH, you can still use: $PYTHON_CMD -m cli.cli${NC}"
    fi
    set -e
}
...
create_config_file() {
    ...
    DISCORD_TOKEN="${DISCORD_TOKEN:-}" \
    LIGHTAGENT_LANG="${INSTALL_LANG:-auto}" \
    $PYTHON_CMD -c "
import json, os
e = os.environ.get
base = {
    ...
    'lightagent_lang': e('LIGHTAGENT_LANG', 'auto'),
...
start_project() {
    echo ""
    echo -e "${GREEN}${EMOJI_ROCKET} Starting LightAgent...${NC}"
    sleep 1

    local USE_LIGHTAGENT=false
    if command -v lightagent &> /dev/null; then
        USE_LIGHTAGENT=true
    fi

    if $USE_LIGHTAGENT; then
        cd "${BASE_DIR}"
        lightagent start --no-logs
    else
        ...
        if [[ "$OS_TYPE" == "Linux" ]]; then
            ...
            echo -e "${GREEN}${EMOJI_LIGHTAGENT} LightAgent started on Linux (using $PYTHON_CMD)${NC}"
        elif [[ "$OS_TYPE" == "Darwin" ]]; then
            nohup $PYTHON_CMD "${BASE_DIR}/app.py" > "${BASE_DIR}/nohup.out" 2>&1 &
            echo -e "${GREEN}${EMOJI_LIGHTAGENT} LightAgent started on macOS (using $PYTHON_CMD)${NC}"
        else
    ...
    echo ""
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${GREEN}${EMOJI_CHECK} $(t "LightAgent 已在后台运行" "LightAgent is now running in background")!${NC}"
    ...
    if $USE_LIGHTAGENT; then
        echo -e "  ${GREEN}lightagent stop${NC}       $(t "停止服务" "Stop the service")"
        echo -e "  ${GREEN}lightagent restart${NC}    $(t "重启服务" "Restart the service")"
        echo -e "  ${GREEN}lightagent status${NC}     $(t "查看状态" "Check status")"
        echo -e "  ${GREEN}lightagent logs${NC}       $(t "查看日志" "View logs")"
        echo -e "  ${GREEN}lightagent update${NC}     $(t "更新并重启" "Update and restart")"
        echo -e "  ${GREEN}lightagent install-browser${NC}  $(t "安装浏览器工具" "Install browser tool")"
    else
    ...
    echo ""
    echo -e "${YELLOW}$(t "提示：需要让 Agent 浏览网页时，运行 lightagent install-browser 安装浏览器工具" "Tip: to let the Agent browse the web, run 'lightagent install-browser' to install the browser tool")${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
show_usage() {
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_LIGHTAGENT} LightAgent Management Script${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
# Check if lightagent CLI is available
has_lightagent() {
    command -v lightagent &> /dev/null
}
...
cmd_start() {
    ...
    if has_lightagent; then
        cd "${BASE_DIR}"
        lightagent start
    else
        if is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} $(t "LightAgent 已在运行中" "LightAgent is already running") (PID: $(get_pid))${NC}"
...
cmd_stop() {
    ...
    if has_lightagent; then
        ...
        echo -e "${GREEN}${EMOJI_STOP} $(t "正在停止 LightAgent" "Stopping LightAgent")...${NC}"

        if ! is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} $(t "LightAgent 未在运行" "LightAgent is not running")${NC}"
            return 0
        fi
        ...
        echo -e "${GREEN}${EMOJI_CHECK} $(t "LightAgent 已停止" "LightAgent stopped")${NC}"
    fi
}
...
cmd_status() {
    if has_lightagent; then
        ...
        echo -e "${CYAN}${BOLD}=========================================${NC}"
        echo -e "${CYAN}${BOLD}   ${EMOJI_LIGHTAGENT} LightAgent Status${NC}"
        echo -e "${CYAN}${BOLD}=========================================${NC}"
...
cmd_config() {
    ...
    select_language
    echo ""
    echo -e "${YELLOW}${EMOJI_WRENCH} $(t "正在重新配置 LightAgent" "Reconfiguring LightAgent")...${NC}"
...
# Update project
cmd_update() {
    echo -e "${GREEN}${EMOJI_WRENCH} $(t "正在更新 LightAgent" "Updating LightAgent")...${NC}"
    cd "${BASE_DIR}"
...
install_mode() {
    ...
    clear
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_LIGHTAGENT} LightAgent Installation${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
# Initialize UI_LANG for management commands: prefer lightagent_lang from an existing
...
init_ui_lang() {
    ...
    local cfg_lang=""
    if [ -f "${BASE_DIR}/config.json" ]; then
        # `|| true`: grep returns 1 when lightagent_lang is absent, which would abort
        # the whole script under `set -e` at the very first management command.
        cfg_lang=$(grep -o '"lightagent_lang"[[:space:]]*:[[:space:]]*"[^"]*"' "${BASE_DIR}/config.json" 2>/dev/null | cut -d'"' -f4 || true)
    fi
    case "$cfg_lang" in
        zh) UI_LANG="zh" ;;
        en) UI_LANG="en" ;;
