const translations: Record<string, Record<string, string>> = {
  zh: {
    ...
    menu_skill_hub: '技能广场',
    // onboarding
    onboarding_welcome_title: '欢迎使用 LightAgent',
    ...
    wechat_group_identity_recovery_pending: '检测到 {count} 个群绑定需要确认，请到群管理页完成身份恢复。',
    wechat_group_qr_tip: '扫码后将在群聊中响应 @ 消息',
    wechat_group_rooms_title: '目标群',
    wechat_group_rooms_hint: '按稳定群 ID 精确限制；群名只用于发现待确认候选。',
    wechat_group_rooms_refresh: '刷新',
    wechat_group_rooms_refreshed: '群列表已刷新',
    wechat_group_rooms_refresh_failed: '刷新群列表失败',
    wechat_group_rooms_empty: '登录后刷新群列表；可在下方填写用于候选发现的群名。',
    ...
    groups_rooms_fallback_hint: '群名只用于发现待确认候选，每行一个群名。',
    ...
    status_starting: '正在启动 LightAgent...',
  ...
  en: {
    ...
    // onboarding
    onboarding_welcome_title: 'Welcome to LightAgent',
    ...
    wechat_group_identity_recovery_pending: '{count} group binding(s) need confirmation. Open Groups to finish identity recovery.',
    wechat_group_qr_tip: 'After login, the channel replies to @ messages in groups',
    wechat_group_rooms_title: 'Target groups',
    wechat_group_rooms_hint: 'Use stable room IDs; names only discover candidates for confirmation.',
    ...
    wechat_group_rooms_empty: 'Refresh after login, or enter candidate group names below.',
    ...
    groups_rooms_fallback_hint: 'Names only discover candidates for confirmation. One group name per line.',
    ...
    status_starting: 'Starting LightAgent...',
...
const LANGUAGE_STORAGE_KEY = 'lightagent_lang'
const LEGACY_LANGUAGE_STORAGE_KEY = 'cow_lang'
...
// in lightagent_lang and always wins. Falls back to 'en' when the locale is unknown.
function detectDefaultLang(): Lang {
  const locale = (window.electronAPI?.systemLocale || navigator.language || '').toLowerCase()
  return locale.startsWith('zh') ? 'zh' : 'en'
}

let savedLang = localStorage.getItem(LANGUAGE_STORAGE_KEY) as Lang | null
if (!savedLang) {
  savedLang = localStorage.getItem(LEGACY_LANGUAGE_STORAGE_KEY) as Lang | null
