...
#
# Output: desktop/build/dist/lightagent-backend/  (folder with the executable)
set -euo pipefail
...
pyinstaller "$BUILD_DIR/lightagent-backend.spec" \
  --noconfirm \
  --distpath "$BUILD_DIR/dist" \
  --workpath "$BUILD_DIR/build-work"

echo ""
echo "==> Done. Bundle at: $BUILD_DIR/dist/lightagent-backend/"
du -sh "$BUILD_DIR/dist/lightagent-backend/" 2>/dev/null || true
echo "==> Smoke test: LIGHTAGENT_DESKTOP=1 \"$BUILD_DIR/dist/lightagent-backend/lightagent-backend\""
