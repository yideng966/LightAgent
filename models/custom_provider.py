...
LightAgent historically supported only a *single* custom provider via the flat
...
def resolve_custom_provider_config(provider_type):
    """Resolve one explicit custom provider without using the active bot_type.

    Voice, image and other independent capabilities can select a custom
    provider that differs from the active chat provider. Unknown multi-
    provider ids intentionally return None instead of falling back to the
    legacy flat fields.
    """
    is_custom, provider_id = parse_custom_bot_type(provider_type)
    if not is_custom:
        return None

    if provider_id:
        provider = _find_provider_by_id(get_custom_providers(), provider_id)
