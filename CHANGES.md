...
- `IMAGE_CREATE` 现在保留去前缀后的原始提示词并跳过文本型 V2 上下文刷新，报告、权限和生图脚本不会再读取摘要、人设或近期群聊中的干扰文本。
- 增加原始提示词、报告门禁隔离和 V2 刷新绕过回归测试。
...
- `tests/test_wechat_group_channel.py`
- `AGENTS.md`
- `plans/20260730_修复微信群生图误判群聊报告.md`

验证记录：

- 微信群主通道 145 项通过；群聊报告、权限、上下文、V2 上下文和意图路由专项 50 项通过。
...
- 修复部分 OpenAI 兼容模型把内部分析直接写入普通流式 `content`、且完全不提供思考标签或 `reasoning_content` 时，微信群仍可能把分析前言作为回复发送并写入会话历史的问题。
- 微信群正文事件改为在整轮流式输出完成并净化后发出；渠道判定优先使用当前请求上下文，避免复用 Agent 时残留的 Web 模型渠道状态绕过过滤。
- 新增高置信度无标签前言识别，仅处理“用户观察 + 自我决策 + 最终答复”结构，兼容空行和单换行分段；没有可确认最终答复时按 fail closed 处理，普通多段答复保持完整。

关键文件：

- `agent/protocol/agent_stream.py`
- `tests/test_agent_stream_thinking_filter.py`
- `plans/20260729_修复无标签思考泄漏并发布2.1.20.md`
- `AGENTS.md`

验证记录：

- 无标签、带标签、跨分片、渠道上下文优先级和正常多段正文定向回归 12 项通过。
- Agent 日志、事件处理、模型故障转移、微信群持久化及拟人化组合 47 项通过。
- `python -X utf8 -m unittest discover -s tests`：共运行 1223 项，结果 OK，1 项按条件跳过。
- Python 语法检查与 `git diff --check` 通过；未修改、部署或重启远端 Docker。

### 修复 IM 流式思考过程泄漏

- Agent 流式正文新增跨 SSE 分片的有状态 `<think>...</think>` 解析，开始标签、结束标签或标签名被拆到不同分片时，微信群等 IM 渠道仍会完整丢弃思考块；未闭合思考块按 fail closed 处理，孤立 `</think>` 不再进入最终回复。
- 流式结束后基于完整原始正文再次净化，再用于回复发送和会话持久化；标准 `reasoning_content` 继续单独处理，不会拼入最终正文。
- Web 在开启思考时继续保留内嵌思考正文并移除原始标签，关闭思考时继续隐藏思考块；普通类标签文本保持不变。

关键文件：

- `agent/protocol/agent_stream.py`
- `tests/test_agent_stream_thinking_filter.py`
- `plans/20260729_修复流式思考泄漏.md`
- `AGENTS.md`

验证记录：

- 修复前新增 6 项回归中 5 项按预期失败；修复后扩展为 8 项并全部通过。
- Agent 流式、事件处理、模型故障转移和微信群持久化组合 38 项通过。
- `python -X utf8 -m unittest discover -s tests`：共运行 1219 项，结果 OK，1 项按条件跳过。
- Python 语法检查与 `git diff --check` 通过；未修改、部署或重启远端 Docker。
...
- 群摘要、永久记忆正文和 Dream 审计摘要明确使用简体中文输出；即使原始群消息或已有记忆为其他语言，也只保留必要的专有名词、产品名、代码和 URL，结构修复重试继续沿用同一中文约束。
...
- 群记忆中文输出定向回归 16 项通过，覆盖两阶段 `zh-CN` 请求、简体中文提示词和非法 JSON 修复重试；共享 Dream 引擎、材料、生成与调度组合 38 项通过。
...
- 修复“群聊 -> 群友画像 -> 自主学习”展开后画像双栏被压缩为零高度、内容互相覆盖的问题；画像区域改为稳定的响应式高度，自主学习进入主滚动流，在桌面双栏和移动端单列下均不再重叠。
- 自主学习改为展开时按需加载，并持久化展开状态、配置草稿和当前异步操作；加载状态、错误重试、未保存提示及按钮禁用状态保持一致，查看差异、保存、执行或回滚不再导致面板关闭和表单值丢失。
- 执行与回滚完成后只刷新一轮配置、状态和运行记录，并在画像确有变更时同步刷新当前群画像；无新消息会显示跳过结果，回滚只对当前最新可回滚的有效运行开放。
- 画像查询增加请求序号、稳定群和查询词三重校验，快速切群或搜索时丢弃迟到响应，避免旧群画像覆盖当前群数据。
- 自主学习文案统一纳入中英文资源，运行 ID、失败原因和差异内容支持安全换行；主要操作按钮、折叠入口和开关标签保持至少 44px 点击区域。
...
- `tests/test_wechat_group_memory_ui.py`
- `tests/test_wechat_group_web.py`
- `plans/20260729_修复群友画像自主学习页面.md`
...
- `node --check channel/web/static/js/console.js` 与 `git diff --check` 通过。
- 使用当前仓库 `python app.py` 启动本地 Web 单通道，在 375、768、1024、1440 宽度及浅色/深色主题完成 Playwright 验收；画像区与自主学习区重叠为 0，无横向溢出，键盘 Enter 可展开，操作控件不小于 44px。
- 浏览器交互验证确认：折叠前不请求自主学习接口；展开后配置、状态和记录各请求一次；差异查看和保存保留展开状态与草稿；执行后状态接口各刷新一次并同步刷新画像。
...
- 修复 wechat4u 已识别扫码入群、但成员或邀请人 ID 查找失败时高层事件被 Wechaty 丢弃的问题：sidecar 改为直接消费结构化 puppet 成员事件并最佳努力补齐联系人；入群成员 ID 全缺失时仍发送不带 mention 的配置欢迎内容，离群成员未知时继续拒绝发送。
...
- 本次缺失 ID 兼容修复：sidecar Node 单元测试 57 项通过；微信群成员消息纯逻辑与通道 Python 定向回归 154 项、客户端与消息协议回归 25 项通过；Python/JavaScript 语法和 `git diff --check` 通过。
...
- 修复 Windows 桌面 Job 在同一 Bash step 执行 `npm run build` 后未继续运行 electron-builder 的问题：源码编译与安装包打包拆成独立步骤，确保 Windows 实际执行 NSIS 打包。
- 将桌面 Actions artifact 的 `if-no-files-found` 从 `ignore` 收紧为 `error`；任何平台缺少安装包都会阻断发布，不再出现 Job 显示成功但 GitHub Release 缺少 `.exe` 的情况。
- 补充强制 `PYTHONIOENCODING=cp1252` 的脚本子进程回归，以及逐 Docker stage 检查 APT 重试配置的静态合同测试。
- `v2.1.7` 和 `v2.1.8` 均已有远端产物，未移动或覆盖原标签；完整桌面产物修复通过新补丁版本 `v2.1.9` 发布，避免同版本对应不同源码和资产内容。
...
- `docs/releases/v2.1.9.md`
- `plans/20260728_修复发布打包失败.md`

验证记录：

- GitHub Actions 原始日志已确认 Windows `UnicodeEncodeError`、ARM64 APT 下载连接重置和 macOS x64 npm `ECONNRESET` 三个独立根因。
- `v2.1.8` Docker 基础版与 `skills-full` 多架构构建成功；桌面重跑后两个 macOS 架构成功，但 Windows Job 未生成 artifact，GitHub Release 复核仅有两个 `.dmg`，从而发现空产物假成功问题。
- 发布版本、Docker 部署和发行说明定向回归 16 项通过；`v2.1.8` 发行说明校验通过。
- Windows 产物完整性修复后，发布版本、Docker 部署和发行说明定向回归 17 项通过；`v2.1.9` 发行说明校验通过。
- `PYTHONUTF8=1 python -m unittest discover -s tests`：1112 项通过，1 项按条件跳过。
- Python 编译、GitHub Actions YAML 解析、JavaScript 语法、配置模板 JSON 和 `git diff --check` 通过。
- 本机 Docker Desktop Linux Engine 未运行，`docker build --check` 无法连接 daemon；Dockerfile 语法与多架构行为留待标签触发的远端 Buildx 构建完成最终验证。
...
- 修正微信群归档查询中无条件使用 `stable_room_id OR room_id` 的越界风险：稳定身份记录只允许按 `stable_room_id` 命中，只有未绑定稳定身份的历史记录才允许按 legacy `room_id` 兼容读取。
- 将严格群作用域应用到最近消息、归档检索、本地摘要、消息精确读取、蒸馏、增量读取、成员列表、群名读取和生图额度统计，阻断同类跨群旁路。
- 新增 runtime/legacy 群 ID 与其他群 stable ID 碰撞场景的回归测试，覆盖总结使用的归档证据、本地抽取摘要和最近消息，并验证旧归档兼容读取不受影响。
...
- 验证结果：`python -m unittest tests.test_skill_registry tests.test_skill_hub_integration` 共 8 项通过；Skill Hub 元数据校验、确定性打包及新地址签名索引生成通过；GitHub Pages 与 `registry.json` 均返回 HTTP 200，LightAgent 在线验签及示例 ZIP SHA-256 校验通过。
