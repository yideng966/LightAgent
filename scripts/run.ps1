...
# ── colours ──────────────────────────────────────────────────────
function Write-LightAgent   { param([string]$M) Write-Host $M -ForegroundColor Green  }
...
# Initialize $UiLang for management commands: prefer lightagent_lang from an existing
...
function Initialize-UiLang {
    ...
    if (Test-Path "$BaseDir\config.json") {
        try {
            ...
            if ($cfg.lightagent_lang) { $cfgLang = "$($cfg.lightagent_lang)" }
        } catch {}
    }
    switch ($cfgLang) {
        "zh"    { $script:UiLang = "zh" }
...
function Assert-Python {
    ...
    Write-LightAgent ((T "找到 Python" "Found Python") + ": $PythonCmd")
}

# ── clone project ────────────────────────────────────────────────
function Install-Project {
    if (Test-Path "LightAgent") {
        # Auto-backup the existing directory without prompting.
        $backup = "LightAgent_backup_$(Get-Date -Format 'yyyyMMddHHmmss')"
        Rename-Item "LightAgent" $backup
        Write-Warn ((T "已存在 LightAgent 目录，已自动备份为" "Existing 'LightAgent' directory backed up to") + " '$backup'")
    }
    ...
    Write-LightAgent (T "正在克隆 LightAgent 项目..." "Cloning LightAgent project...")
    $cloneOk = $false
    ...
    try {
        ...
        Write-LightAgent (T "GitHub 可达，正在从 GitHub 克隆..." "GitHub is reachable, cloning from GitHub...")
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git clone --depth 10 --progress "https://github.com/yideng966/LightAgent.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
        if (-not $cloneOk) {
            if (Test-Path "LightAgent") { Remove-Item -Recurse -Force "LightAgent" }
        }
    } catch {}

    if (-not $cloneOk) {
        Write-Warn (T "GitHub 克隆失败或超时，正在重试..." "GitHub clone failed or timed out, retrying...")
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        git clone --depth 10 --progress "https://github.com/yideng966/LightAgent.git" 2>&1 | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -eq 0) { $cloneOk = $true }
        $ErrorActionPreference = $prevEAP
    ...
    if (-not $cloneOk) {
        Write-Err (T "GitHub 克隆重试失败，请检查网络连接。" "GitHub clone failed after retry. Please check your network connection.")
        Write-Err (T "你也可以手动克隆: git clone https://github.com/yideng966/LightAgent.git" "You can also manually clone: git clone https://github.com/yideng966/LightAgent.git")
        Read-Host (T "按回车退出" "Press Enter to exit")
        exit 1
    }

    Set-Location "LightAgent"
    $script:BaseDir = $PWD.Path
    $script:IsProjectDir = $true
    Write-LightAgent ((T "项目已克隆" "Project cloned") + ": $BaseDir")
}
...
function Install-Dependencies {
    Write-LightAgent (T "正在安装依赖..." "Installing dependencies...")
    ...
    Write-LightAgent (T "正在注册 lightagent CLI..." "Registering lightagent CLI...")
    $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    & $PythonCmd -m pip install -e $BaseDir @pipMirror 2>&1 | Out-Null
    $ErrorActionPreference = $prevEAP

    # Add the Scripts dir (where pip puts lightagent.exe) to PATH, both for this
    # session and persistently, so `lightagent` keeps working after a reboot.
    $scriptsDir = & $PythonCmd -c "import sysconfig; print(sysconfig.get_path('scripts'))" 2>$null
    if ($scriptsDir -and (Test-Path $scriptsDir)) {
        Add-ScriptsDirToPath $scriptsDir
    }

    $lightAgentBin = Get-Command lightagent -ErrorAction SilentlyContinue
    if ($lightAgentBin) {
        Write-LightAgent ((T "lightagent CLI 注册成功" "lightagent CLI registered") + ": $($lightAgentBin.Source)")
    } else {
        Write-Warn ((T "lightagent CLI 不在 PATH 中，你可以使用" "lightagent CLI not in PATH. You can use") + ": $PythonCmd -m cli.cli")
    }
}

# Add the Python Scripts dir to PATH for this session and persist it to the
# user PATH (User scope needs no admin rights), so `lightagent` survives a reboot.
function Add-ScriptsDirToPath {
    ...
    try {
        ...
        if (-not $already) {
            ...
            Write-LightAgent ((T "已将 lightagent 命令目录加入用户 PATH（重启后仍可用）" "Added lightagent command directory to user PATH (persists after reboot)") + ": $ScriptsDir")
        }
    } catch {
...
function Configure-Model {
    ...
    Write-LightAgent ((T "正在配置" "Configuring") + " $($m.Provider)...")
...
function Configure-Channel {
    ...
    switch ($script:ChannelChoice) {
        ...
        "feishu" {
            ...
            Write-LightAgent (T "配置飞书（WebSocket 模式）..." "Configure Feishu (WebSocket mode)...")
        ...
        "dingtalk" {
            ...
            Write-LightAgent (T "配置钉钉..." "Configure DingTalk...")
        ...
        "wecom_bot" {
            ...
            Write-LightAgent (T "配置企微智能机器人..." "Configure WeCom Bot...")
        ...
        "qq" {
            ...
            Write-LightAgent (T "配置 QQ 机器人..." "Configure QQ Bot...")
        ...
        "wechatcom_app" {
            ...
            Write-LightAgent (T "配置企微自建应用..." "Configure WeCom App...")
        ...
        "telegram" {
            ...
            Write-LightAgent (T "配置 Telegram..." "Configure Telegram...")
        ...
        "slack" {
            ...
            Write-LightAgent (T "配置 Slack..." "Configure Slack...")
        ...
        "discord" {
            ...
            Write-LightAgent (T "配置 Discord..." "Configure Discord...")
...
function New-ConfigFile {
    Write-LightAgent (T "正在生成 config.json..." "Generating config.json...")

    $config = [ordered]@{
        channel_type              = if ($script:ChannelType) { $script:ChannelType } else { "web" }
        model                     = if ($script:ModelName)  { $script:ModelName }  else { "" }
        lightagent_lang                  = if ($script:InstallLang) { $script:InstallLang } else { "auto" }
        open_ai_api_key           = ""
    ...
    Write-LightAgent (T "配置文件创建成功。" "Configuration file created.")
}

# Resolve the `lightagent` command, self-healing PATH if needed. When `lightagent` is missing
...
function Resolve-LightAgentCommand {
    if (Get-Command lightagent -ErrorAction SilentlyContinue) { return $true }

    $py = if ($PythonCmd) { $PythonCmd } else { Find-Python }
    if ($py) {
        $scriptsDir = & $py -c "import sysconfig; print(sysconfig.get_path('scripts'))" 2>$null
        if ($scriptsDir -and (Test-Path "$scriptsDir\lightagent.exe")) {
            Add-ScriptsDirToPath $scriptsDir
            if (Get-Command lightagent -ErrorAction SilentlyContinue) { return $true }
        }
    }
...
# ── start via lightagent CLI ─────────────────────────────────────────────
function Start-LightAgent {
    Write-LightAgent (T "正在启动 LightAgent..." "Starting LightAgent...")
    if (Resolve-LightAgentCommand) {
        & lightagent start
    } else {
        Write-Warn (T "未找到 lightagent CLI，直接启动..." "lightagent CLI not found, starting directly...")
        & $PythonCmd "$BaseDir\app.py"
    }
}

# ── delegate management commands to lightagent CLI ──────────────────────
function Invoke-LightAgentCommand {
    param([string]$Cmd)
    if (Resolve-LightAgentCommand) {
        & lightagent $Cmd
    } else {
        # Fall back to the module entrypoint so management commands still work
        # even when lightagent.exe isn't on PATH (e.g. right after a fresh reboot on
        # an older install).
        $py = if ($PythonCmd) { $PythonCmd } else { Find-Python }
        if ($py -and (Test-Path "$BaseDir\app.py")) {
            Write-Warn (T "未找到 lightagent 命令，使用 python -m cli.cli 兜底运行..." "lightagent command not found, falling back to python -m cli.cli...")
            Push-Location $BaseDir
            try { & $py -m cli.cli $Cmd } finally { Pop-Location }
        } else {
            Write-Err (T "未找到 lightagent CLI，请先不带参数运行本脚本进行安装。" "lightagent CLI not found. Run this script without arguments first to install.")
            exit 1
        }
    }
}
...
function Show-Usage {
    Write-Info "========================================="
    Write-Info "   LightAgent Management Script (Windows)"
    Write-Info "========================================="
    Write-Host ""
...
function Install-Mode {
    Clear-Host
    Write-Info "========================================="
    Write-Info "   LightAgent Installation (Windows)"
    Write-Info "========================================="
    Write-Host ""
    ...
    if ($IsProjectDir) {
        Write-LightAgent (T "检测到已有项目目录。" "Detected existing project directory.")
        if (Test-Path "$BaseDir\config.json") {
            Write-LightAgent (T "项目已配置。" "Project already configured.")
            Write-Host ""
            Show-Usage
            return
        }
    ...
    Write-Host ""
    if ($script:AccessInfo) { Write-LightAgent $script:AccessInfo }
    Write-Warn (T "提示：需要让 Agent 浏览网页时，运行 lightagent install-browser 安装浏览器工具" "Tip: to let the Agent browse the web, run 'lightagent install-browser' to install the browser tool")
    Start-LightAgent
}

# ── update ────────────────────────────────────────────────────────
function Update-Project {
    Write-LightAgent (T "正在更新 LightAgent..." "Updating LightAgent...")
    ...
    if ($lightAgentBin) {
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        & lightagent stop 2>&1 | Out-Null
        $ErrorActionPreference = $prevEAP
    }

    if (Test-Path "$BaseDir\.git") {
        Write-LightAgent (T "正在拉取最新代码..." "Pulling latest code...")
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        ...
        if ($pullExit -ne 0) {
            ...
            git remote set-url origin https://github.com/yideng966/LightAgent.git 2>&1 | Out-Null
            git pull 2>&1 | Out-Null
            $ErrorActionPreference = $prevEAP
        }
    } else {
    ...
    # Start via python -m cli.cli instead of lightagent.exe, because the exe may
...
switch ($Command.ToLower()) {
    ""        { Install-Mode }
    "start"   { Invoke-LightAgentCommand "start" }
    "stop"    { Invoke-LightAgentCommand "stop" }
    "restart" { Invoke-LightAgentCommand "restart" }
    "status"  { Invoke-LightAgentCommand "status" }
    "logs"    { Invoke-LightAgentCommand "logs" }
    "config"  {
        Assert-Python
        Install-Dependencies
        Select-Model
