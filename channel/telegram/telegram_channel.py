...
Implementation note:
    ...
    thread with its own asyncio loop so the rest of lightagent (which is sync)
    stays untouched. Inbound updates are dispatched onto lightagent's existing
...
class TelegramChannel(ChatChannel):
    ...
    def startup(self):
        ...
        # Run the asyncio event loop in a dedicated thread so the sync lightagent body
        # is untouched.
    ...
    async def _on_message(self, update, _context):
        ...
        try:
            ...
            # Dispatch to lightagent main pipeline (reuses ChatChannel._compose_context routing)
    ...
    def send(self, reply: Reply, context: Context):
        """Called from lightagent's sync main thread; we marshal the coroutine onto the loop thread."""
