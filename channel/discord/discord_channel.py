...
Implementation note:
    ...
    with its own asyncio loop so the rest of lightagent (which is sync) stays
    untouched. Inbound messages are dispatched onto lightagent's existing sync
...
class DiscordChannel(ChatChannel):
    ...
    def send(self, reply: Reply, context: Context):
        """Called from lightagent's sync main thread; marshal the coroutine onto the loop thread."""
