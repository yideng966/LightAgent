...
Implementation note:
    ...
    threads. We start it in a dedicated thread so the rest of lightagent (sync) stays
    untouched. Inbound events are dispatched onto lightagent's existing sync
...
class SlackChannel(ChatChannel):
    ...
    def send(self, reply: Reply, context: Context):
        """Called from lightagent's sync main thread; Slack Web client is sync-safe."""
