"""
LightAgentCli plugin - Intercept lightagent/slash commands in chat messages.
...
# Known top-level subcommands that lightagent supports.
...
@plugins.register(
    name="lightagent_cli",
    desc="Handle lightagent/slash commands in chat messages",
    version="0.1.0",
    author="LightAgent",
    desire_priority=1000,
)
class LightAgentCliPlugin(Plugin):

    def __init__(self):
        super().__init__()
        self.handlers[Event.ON_HANDLE_CONTEXT] = self.on_handle_context
        self.aliases = self._build_aliases()
        logger.debug("[LightAgentCli] initialized")
    ...
    def _build_aliases() -> dict:
        ...
        try:
            ...
            logger.warning(f"[LightAgentCli] could not read command_aliases from config: {e}")
            overrides = {}
        if not isinstance(overrides, dict):
            logger.warning(f"[LightAgentCli] command_aliases must be an object, got {type(overrides).__name__}; ignoring")
            overrides = {}
        for key, value in overrides.items():
            if not isinstance(key, str) or not isinstance(value, str):
                logger.warning(f"[LightAgentCli] ignoring non-string alias entry: {key!r} -> {value!r}")
                continue
            k = key.strip().lower()
            if k:
        ...
        for key, value in merged.items():
            ...
            if head not in KNOWN_COMMANDS:
                logger.warning(f"[LightAgentCli] dropping alias '/{key}' -> '{value}': unknown command '{head}'")
                continue
            if key in KNOWN_COMMANDS:
                logger.warning(f"[LightAgentCli] alias '/{key}' shadows a real command; the command takes precedence")
            valid[key] = value
        return valid
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if kind == "run":
            _, cmd, args = result
            logger.info(f"[LightAgentCli] intercepted command: {cmd} {args}")
    ...
    def _parse_command(self, content: str):
        """
        Parse lightagent command from message text.

        Supported formats:
          lightagent <command> [args...]   e.g. "lightagent skill list"
        ...
        if content.startswith("lightagent "):
            ...
            if cmd not in KNOWN_COMMANDS:
                # 'lightagent xxx' that isn't a command — don't intercept (could be
                # natural language like "lightagent xxx 怎么样").
                return None
            args = parts[1] if len(parts) > 1 else ""
            return cmd, args
    ...
    def _resolve(self, token: str, user_args: str):
        ...
        is trivially unit-testable. Note the `lightagent ` form never reaches stages
    ...
    def execute(self, query: str, session_id: str = "") -> str:
        """Execute a lightagent/slash command string without a channel context.
    ...
    def _dispatch(self, cmd: str, args: str, e_context: EventContext, session_id: str = "") -> str:
        if cmd in CLI_ONLY_COMMANDS:
            return _t(
                f"⚠️ `lightagent {cmd}` 只能在命令行终端中执行。\n请在终端运行: lightagent {cmd}",
                f"⚠️ `lightagent {cmd}` can only run in a terminal.\nRun it in your shell: lightagent {cmd}",
            )
        ...
        if handler:
            try:
                ...
                logger.error(f"[LightAgentCli] command '{cmd}' failed: {e}")
    ...
    def _cmd_help(self, args: str, e_context, **_) -> str:
        if _t("zh", "en") == "en":
            lines = [
                ...
                "",
                "💡 You can also use lightagent <command> instead of /<command>",
            ]
        else:
            lines = [
                "📋 LightAgent 命令列表",
                ...
                "",
                "💡 也可以用 lightagent <command> 代替 /<command>",
            ]
        return "\n".join(lines)

    def _cmd_version(self, args: str, e_context, **_) -> str:
        return f"LightAgent v{__version__}"
    ...
    def _cmd_status(self, args: str, e_context: EventContext, session_id: str = "", **_) -> str:
        ...
        cfg = conf()
        lines = [_t("📊 LightAgent 运行状态", "📊 LightAgent Status"), ""]
    ...
    def _config_set(self, key: str, value_str: str) -> str:
        ...
        logger.info(f"[LightAgentCli] config update: {updates}, synced envs: {synced_envs}")

        try:
            load_config()
        except Exception as e:
            logger.warning(f"[LightAgentCli] config reload warning: {e}")
    ...
    # ------------------------------------------------------------------
    # install-browser (shared logic with lightagent install-browser CLI)
    # ------------------------------------------------------------------
    ...
    def _send_install_progress(e_context, text: str) -> None:
        ...
        if e_context is None:
            logger.info(f"[LightAgentCli] install-browser: {text}")
            return
        try:
            ...
            logger.warning(f"[LightAgentCli] install-browser progress send failed: {e}")
    ...
    def _cmd_install_browser(self, args: str, e_context, **_) -> str:
        ...
        if args.strip():
            return _t(
                "用法: /install-browser\n\n"
                "无需参数，等同于终端执行 `lightagent install-browser`。\n"
                "安装过程可能持续数分钟；进度会以多条消息推送，pip 详细输出见服务日志。",
                "Usage: /install-browser\n\n"
                "No arguments needed; equivalent to running `lightagent install-browser` in a terminal.\n"
        ...
        if code != 0:
            return _t(
                ...
                "也可在终端执行 `lightagent install-browser`。",
                "❌ Installation did not finish successfully. Check the messages above or the server log; "
                "you can also run `lightagent install-browser` in a terminal.",
            )
        return _t(
            "✅ 安装流程已结束。请重启 LightAgent 后使用 browser 工具（进度见上方消息）。",
            "✅ Installation finished. Restart LightAgent to use the browser tool (see messages above for progress).",
        )
    ...
    def _refresh_skill_manager(self):
        ...
        try:
            ...
            logger.debug(f"[LightAgentCli] skill refresh skipped: {e}")
    ...
    def _memory_dream(self, days: int, e_context, session_id: str) -> str:
        ...
        def _run():
            try:
                ...
                logger.warning(f"[LightAgentCli] /memory dream failed: {e}")
    ...
    def _memory_dream_sync(self, flush_mgr, days: int) -> str:
        ...
        try:
            ...
            logger.warning(f"[LightAgentCli] /memory dream sync failed: {e}")
    ...
    def _memory_rebuild_index(self, e_context, session_id: str) -> str:
        ...
        try:
            ...
            logger.exception("[LightAgentCli] /memory rebuild-index: build provider failed")
        ...
        def _run():
            try:
                ...
                logger.exception("[LightAgentCli] /memory rebuild-index failed")
    ...
    def _memory_rebuild_sync(memory_manager, model_label, dim_label) -> str:
        ...
        try:
            ...
            logger.exception("[LightAgentCli] /memory rebuild-index sync failed")
    ...
    def _notify(e_context, text: str):
        ...
        if e_context is None:
            logger.info(f"[LightAgentCli] {text}")
            return
        try:
            channel = e_context["channel"]
            context = e_context["context"]
            ...
            logger.warning(f"[LightAgentCli] notify failed: {e}")
    ...
    def _create_standalone_flush_manager():
        ...
        workspace = Path(expand_path(conf().get("agent_workspace", "~/lightagent")))
    ...
    def _knowledge_stats(self) -> str:
        ...
        knowledge_dir = os.path.join(
            expand_path(conf().get("agent_workspace", "~/lightagent")),
            "knowledge"
        )
    ...
    def get_help_text(self, **kwargs):
        return _t("在对话中使用 /help 或 lightagent help 查看可用命令", "Use /help or lightagent help in chat to see available commands")
